...
var verifrom = {
    ...
    Request:function(params) {
        ...
        {
            console.log('loadParams - Exception including parameters :',err);
        }
    },
    request: {
        queue:function() {
...
verifrom.worker.prototype = {
    ...
    postMessage:function(message,options) {
        try {
            this.worker.postMessage({"channel":options.channel,"payload":message});
        } catch(e) {
            verifrom.console.log(1,'postMessage error');
        }
    },
    emit:function (channel,message) {
        try {
            this.worker.postMessage({"channel":channel, "payload":message});
        } catch(e)
        {
            verifrom.console.error(1,'emit error');
        }
    }
};
