...
{
  // DomI Plus DM version -----------------------------------------------------
  let absolutePath = Components.stack.filename;
  const dm_js= absolutePath.substring(0, absolutePath.lastIndexOf("/") + 1) + "DM_version/DM_init.js"; 
  Services.scriptloader.loadSubScript( dm_js+"?_"+Math.random(), window)
  // run init() !
  DM_init();
  ...
  const pref_name= "extensions.domInspectorPlusDm.smallMargin";
  
  try {
      if( Services.prefs.prefHasUserValue( pref_name ) && 
          Services.prefs.getBoolPref(      pref_name ) ) {

        document.getElementById("winInspectorMain").setAttribute( "chromemargin", "17,0,0,0");
        // add border to #bxInspectorMain 
      }
	  
	  // to DRAG-WINDOW! ----------
	  const my_hbox= document.createElement("hbox")
	  my_hbox.id="dmVersion_menubar_space";
	  document.getElementById("mbrInspectorMain").insertBefore(my_hbox, null);
	  
  } catch (ex) {
    window.console.info(" ignoring error in  '"+pref_name+"': "+ex);
  };

}


function DomiPlusDm_improveViewersMenu( mObjectPanel )
{
    const pref_key= "extensions.domInspectorPlusDm.keyViewers";

    try {
        var prefValue= '{"Computed Style": "E"}';
        if( Services.prefs.prefHasUserValue(         pref_key) ){
            prefValue= ""+Services.prefs.getCharPref(pref_key);
        }
        var persistedAttrs= null;
        if( prefValue.trim().length > 0 ){
            persistedAttrs= JSON.parse( prefValue );
        }
        if( persistedAttrs ){
            mObjectPanel.rbak= mObjectPanel.rebuildViewerList; 
            mObjectPanel.rebuildViewerList= function(pop){
                this.rbak(pop)
                if(pop){
                    for( var sLabel in persistedAttrs ){
                        var key= persistedAttrs[ sLabel ];
                        
                        var mitem= pop.querySelector("menuitem[label='"+sLabel+"']");
                        if( mitem ){
                            mitem.setAttribute("accesskey", ""+key );
                        }
                    }
                }
            }
        ...
        window.console.info(" ignoring error in  '"+pref_key+"': "+ex );
    };
}
...
InspectorApp.prototype =
{
  ...
  initViewerPanels: function IA_InitViewerPanels()
  {
    ...
    // DM Version -----------------------------------------------------------------------
    DomiPlusDm_improveViewersMenu( this.mObjectPanel );
