...
//Las variables se inicializan en empty porque cuando se pasen al siguiente programa comprueba si tienen este valor y las ignora.
var pathemail = "empty"; //Aqui ira el path donde esta el email creado aqui
var pathtemp = "empty";
var time = new Date();	
var subject = "empty";
...
var funciones = 
{
	SaveMail: function (tAnex)
	{
		if ((gFolderDisplay != null) && (gFolderDisplay.selectedCount > 0)) //Si hay como mínimo un mensaje seleccionado.
		{
			...
			try
			{
				//Este if es el que comprueba donde esta el mail si en la bandeja de salida o no, busca en la URI del mensaje si contiene el nombre de la carpeta de enviados.
...
{		
	...
	if (file.exists())
	{
		...
		admintags.añadirtag(messageHeader);
	}else
	{
