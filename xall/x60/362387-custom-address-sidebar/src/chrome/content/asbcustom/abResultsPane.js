...
function SetAbView(aURI)
{
  ...
  UpdateSortIndicators_2nd(actualSortColumn, sortDirection);
...
{
  UpdateSortIndicators_2nd(sortColumn, sortDirection);

  if (gAbView)
    gAbView.sortBy(sortColumn, sortDirection);
}

// Change function name from 'UpdateSortIndicators' to
// 'UpdateSortIndicators_2nd'.If original function name
// exists, it deletes thread tree in mail list.
// by shimamu.
function UpdateSortIndicators_2nd(colID, sortDirection)
{
