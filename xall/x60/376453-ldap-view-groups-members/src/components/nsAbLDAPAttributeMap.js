...
const NS_ABLDAPATTRIBUTEMAP_CID = Components.ID(
  "{127b341a-bdda-4270-85e1-edff569a9b80}");
const NS_ABLDAPATTRIBUTEMAPSERVICE_CID = Components.ID(
  "{4ed7d5e1-8800-40da-9e78-c4f509d7ac50}");
...
nsAbLDAPAttributeMap.prototype = {
  ...
  getAllCardAttributes: function getAllCardAttributes() {
    var attrs = [];
    for (var attrName of Object.keys(this.mPropertyMap)) {
      attrs = attrs.concat(this.mPropertyMap[attrName]);
    }
    if (!attrs.length) {
      throw Components.results.NS_ERROR_FAILURE;
    }
      ...
      for (var attr of this.mPropertyMap[prop]) {
        ...
        if (msgAttrs.indexOf(attr) != -1) {
          try {
			...
			aCard.setProperty(prop + '.all', values);
  ...
  checkState: function checkState() {
    var attrsSeen = [];
    for (var attrName of Object.keys(this.mPropertyMap)) {
      let attrArray = this.mPropertyMap[attrName];
	  for (var attr of attrArray) {
		  
		  //Components.utils.reportError('attr: '+attr);
