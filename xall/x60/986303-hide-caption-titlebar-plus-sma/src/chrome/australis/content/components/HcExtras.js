...
var HcExtras_class= function( _win ){
	...
	const Vert_Max= new (function(){
		...
		const elem_browserPanel= document.getElementById("browser-panel");
		...
		this.window_moved= function( value, oldValue, _msgCaller) {
	  	...
	  	this.window_moved_NOW= function( value, oldValue, _msgCaller) {
			...
			if( Vert_Max.snap_right && !Vert_Max.snap_left ){ // ONLY right!
				const leftOuterGap = elem_browserPanel.boxObject.screenX - window.screenX;
				target_X = screen.availLeft + screen.availWidth + bWidth_right /* window.devicePixelRatio*/ - leftOuterGap - elem_browserPanel.boxObject.width;
			}
			if( Vert_Max.snap_left  ){
				target_X = screen.availLeft                     - bWidth_left  /* window.devicePixelRatio*/ + (window.screenX    - elem_browserPanel.boxObject.screenX);
				//myDumpToConsole( "       w.sX: "+window.screenX+", elem.sX: "+elem_browserPanel.boxObject.screenX+" ) " );
			}
			...
			if( !Vert_Max.snap_top && Vert_Max.snap_bottom ){
				const bottomGap= window.screenY + window.outerHeight - elem_browserPanel.boxObject.screenY - elem_browserPanel.boxObject.height;
			...
			if( Vert_Max.snap_top && Vert_Max.snap_bottom ){
				var rzBy_H  = screen.availHeight - elem_browserPanel.boxObject.height;
				//myDumpToConsole( "     ..    w_outerH: "+window.outerHeight+",  elem_h: "+elem_browserPanel.boxObject.height+" ) " );
				if( rzBy_H != 0 ){
			...
			if( Vert_Max.snap_left && Vert_Max.snap_right ){
				var rzBy_W  = screen.availWidth - elem_browserPanel.boxObject.width + bWidth_left + bWidth_right + 1; // mas 1 p/ q agarre bien el scrollbar vert!
				//myDumpToConsole( "     ..    w_outerW: "+window.outerWidth+",  elem_w: "+elem_browserPanel.boxObject.width+" ) " );
				if( rzBy_W != 0 ){
			...
			if( Vert_Max.snap_right ){
				...
				const gapInnerLeft = elem_browserPanel.boxObject.screenX - HideCaption.mainW.boxObject.screenX + HideCaption.mainW.boxObject.x; 
				
				const gapInnerRight= window.innerWidth - elem_browserPanel.boxObject.width - gapInnerLeft + bWidth_right
					- local_offset_devPR;
			...
			if( Vert_Max.snap_left  ){
				...
				const gapInnerLeft = elem_browserPanel.boxObject.screenX - HideCaption.mainW.boxObject.screenX + HideCaption.mainW.boxObject.x + bWidth_left
					- local_offset_devPR; 
