...
//    	}
	...
	this.conditional_set_winbkg_color= function( bReset ){ // bReset  <---- Reset & add-a-DELAY!! --> lwtheme itself changed or lwt-related-option changed! 
		...
		function do__conditional_set_winbkg_color(){
			...
			// NICE lwtheme catch! ...querySelectorAll_forEach("#tab-view-deck:-moz-lwtheme,  #hctp-toolsubparent_box", ... )
			...
			"#hctp-toolsubparent_box"+lwtheme_filter_str+"" ,
				function(elem){
					if( bSet ){
						...
						if( elem.id == "tab-view-deck"){
							elem.style.setProperty("box-shadow", "0 0 0 20px "+HideCaption.winbkg_color, "");
						}
						color_done= true;
					}else{
						elem.style.removeProperty("background-color");
