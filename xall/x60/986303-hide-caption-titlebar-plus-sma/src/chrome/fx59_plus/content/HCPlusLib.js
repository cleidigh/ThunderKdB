...
var HCPlusLib_class= function( _win ){
	...
	this.hc_shutdown= function() {
		...
		try {
			...
			/**
			style_removeProperty( document.getElementById("hctp_special_bkg")                 , "background-color");
			style_removeProperty( document.getElementById("hctp_special_bkg")                 , "box-shadow");
			**/
...
//			}
	...
	*/
	...
	this.set_sysborder_settings=  function(){
		...
		// afecta tboxhide - HideCaption.mainW.setAttribute("hctp_show_newCustBorders", (icm == 0 || icm == 7) && !HCPlusLib.sysborder_real ? "true": "false");
	...
	*/
	...
	this.openUILink   = function( _win, _url, event, aIgnoreButton, aIgnoreAlt ) {
		if( !"openTrustedLinkIn" in _win ||
			 aIgnoreButton && typeof aIgnoreButton == "object" ) { // check OBJECT param for fx63+

			_win.openUILink( _url, event, aIgnoreButton, aIgnoreAlt );
		} else {
			let params= {};
			params.ignoreButton = aIgnoreButton;
			params.ignoreAlt    = aIgnoreAlt;

			// fx 63+ needs "openTrustedLink()"
			if(!params.triggeringPrincipal) { 
				params.triggeringPrincipal = Services.scriptSecurityManager.getSystemPrincipal();
			}
			
			_win.openUILink( _url, event, params );
		}
	};
	
	this.openUILinkIn = function( _win, _url, _where ) {
		if( "openTrustedLinkIn" in _win ){
			_win.openTrustedLinkIn( _url, _where );  // avoid 'openUILinkIn' error in fx63+
		}else{
