...
var MailmindrListView = MailmindrListView || class MailmindrListView {
    constructor(aName) {
        this.treeBox = null;
        this.selection = null;
        this.data = new Array();
    ...
    updateItems(mindrs) {
        ...
        mindrs.forEach(mindr => {
            if (this.data.find(item => mindr.mailmindrGuid === item.mailmindrGuid) === undefined) {
                added.push(mindr);
            } else {
        ...
        let deleted = this.data.filter(function(mindr) {
            return !Boolean(
                unchanged.find(item => item.mailmindrGuid === mindr.mailmindrGuid)
                );
        });
        ...
        this._logger.log(`status: unchaned: ${unchanged.length}, added: ${added.length}, deleted: ${deleted.length}`);
    }
