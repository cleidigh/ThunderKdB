...
var mailmindrPrefPaneInboxZero = {
    ...
    _createFolderPicker: function(parent, pickerId) {
        ...
        let emptyLabel = mailmindrI18n.getString('mailmindr.prefpane.inboxzero.selectfolder.label');
    ...
    loadPreferences: function() {
        ...
        const serializedAccounts = mailmindrCore.readPreference('common.inboxZeroAccounts', '');
        const serializedData = mailmindrCore.readPreference('common.inboxZeroPreferences', '');

        const keys = serializedAccounts.length > 0 ? JSON.parse(serializedAccounts) : [];
        let data = serializedData.length > 0 ? JSON.parse(serializedData) : {};

        for (let key of keys) {
            this._logger.log(`loadPreferences key '${key}' of ${keys}`);
            const item = data[key];
            this._logger.log(`loadPreferences key '${key}' resolved to item: '${item}', folder: '${item && item.folderURI}'`);
            ...
            if (target) {
                this._logger.log(`trying to set value ${item && item.folderURI} to ${target}`);
                mailmindrPreferencesDialog.setValueToElement(target, item.folderURI);
