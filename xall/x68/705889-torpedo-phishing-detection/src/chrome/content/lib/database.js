...
var torpedoOptions = {
	...
	inBlacklist: function (domain, list) {
		var blacklistDomainArr = DB.getBlacklistDomains();
		/*blacklistDomainArr.then(function (blacklistDomains) {
			consoleService.logStringMessage("Domain " + domain);
			consoleService.logStringMessage("Domains " + blacklistDomains);
	...
	startProcessDom: async function (blackListDomainArr) {
		var blacklistWasUpdated = torpedo.prefs.getBoolPref("blacklistWasUpdated");
		if (blackListDomainArr.length > 0) {
			var start = performance.now();
			DB.updateBlacklist(blackListDomainArr);
			var time = performance.now();
			consoleService.logStringMessage('Dauer: ' + (time - start) + ' ms.');
			torpedo.processDOM(blackListDomainArr);
		} else {
			var blacklistArr = await DB.getBlacklistDomains();
			blacklistArr.then(function (blackListDomains) {
				torpedo.blacklistDomainArr = blackListDomains;
				torpedo.processDOM(blackListDomains);
			});
		}
	...
	readInBlacklist: function () {
		this.requestBlacklist(this.startProcessDom);
	},
	...
	requestBlacklist: function (callback, start) {
		...
		ctcBlacklistRequest.onreadystatechange = function () {
			consoleService.logStringMessage("Ready state " + ctcBlacklistRequest.readyState);
			// Blacklist has changed
			if (ctcBlacklistRequest.readyState === 4) {
				if (ctcBlacklistRequest.status === 200) {
					...
					if (contentType == "text/plain") {
						...
						blacklistDomainArr.push("stackoverflow.com");
						torpedo.prefs.setIntPref("currentCtcBlacklistVersion", currentCtcBlacklistVersion);
						var time = performance.now();
						consoleService.logStringMessage('Dauer: ' + (time - start) + ' ms.');
						callback.apply(this, [blacklistDomainArr]);
						consoleService.logStringMessage("Here");
					}
				} else {
					consoleService.logStringMessage("Status " + ctcBlacklistRequest.status);
					callback.apply(this, [blacklistDomainArr]);
				}
			}
		}
