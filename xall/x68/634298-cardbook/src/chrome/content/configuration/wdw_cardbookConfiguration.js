...
if ("undefined" == typeof(wdw_cardbookConfiguration)) {
	...
	var wdw_cardbookConfiguration = {
		...
		loadTitle: function () {
			document.title = ConversionHelper.i18n.getMessage("cardbookPrefTitle") + " (" + cardbookRepository.addonVersion + ")";
		},
		...
		loadAdrFormula: function () {
			...
			var myLabel = [];
			myLabel.push("{{1}} : " + ConversionHelper.i18n.getMessage("postOfficeLabel"));
			myLabel.push("{{2}} : " + ConversionHelper.i18n.getMessage("extendedAddrLabel"));
			myLabel.push("{{3}} : " + ConversionHelper.i18n.getMessage("streetLabel"));
			myLabel.push("{{4}} : " + ConversionHelper.i18n.getMessage("localityLabel"));
			document.getElementById('adrFormula7').value = myLabel.join("    ");
			myLabel = [];
			myLabel.push("{{5}} : " + ConversionHelper.i18n.getMessage("regionLabel"));
			myLabel.push("{{6}} : " + ConversionHelper.i18n.getMessage("postalCodeLabel"));
			myLabel.push("{{7}} : " + ConversionHelper.i18n.getMessage("countryLabel"));
		...
		resetCalendarEntryTitle: function () {
			document.getElementById('calendarEntryTitleTextBox').value = ConversionHelper.i18n.getMessage("eventEntryTitleMessage");
			cardbookRepository.cardbookPreferences.setStringPref("extensions.cardbook.eventEntryTitle", ConversionHelper.i18n.getMessage("eventEntryTitleMessage"));
		},
		...
		getEmailAccountName: function(aEmailAccountId) {
			if (aEmailAccountId == "allMailAccounts") {
				return ConversionHelper.i18n.getMessage(aEmailAccountId);
			}
					...
					for (let k = 0; k < wdw_cardbookConfiguration.allTypes[myABType][myType].length; k++) {
						...
						for (let l = 0; l < cardbookRepository.cardbookCoreTypes[myABType][myType].length; l++) {
							...
							if (myCode == myCoreCodeType) {
								if (myLabel != ConversionHelper.i18n.getMessage(myCoreCodeType)) {
		...
		loadDateDisplayedFormat: function () {
			var labelLong = ConversionHelper.i18n.getMessage("dateDisplayedFormatLong");
			var labelShort = ConversionHelper.i18n.getMessage("dateDisplayedFormatShort");
			var myDate = new Date();
		...
		saveInstantApply: function (aNode) {
			...
			switch (prefType) {
				case "bool":
					...
					break;
				case "string":
					cardbookRepository.cardbookPreferences.setStringPref(prefName, aNode.getAttribute("value"));
					break;
				case "number":
