	...
	var wdw_birthdaySync = {
		...
		do_refresh: function () {
			...
			if (cardbookBirthdaysUtils.lBirthdayList.length == 0) {
				...
				document.title=cardbookRepository.strBundle.formatStringFromName("syncListWindowLabelEnded", [0,0], 2);
			} else {
				...
				if (totalRecordsToInsert != totalRecordsInserted) {
					...
					document.title=cardbookRepository.strBundle.formatStringFromName("syncListWindowLabelRunning", [lTotalDisplayed, totalRecordsToInsert.toString()], 2);
				} else {
					document.title=cardbookRepository.strBundle.formatStringFromName("syncListWindowLabelEnded", [totalRecordsInserted.toString(), totalRecordsToInsert.toString()], 2);
				}
			}
		}
	};
