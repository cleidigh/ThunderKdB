			...
			if (myExtension.toLowerCase() == "vcf") {
				...
				var listener_loadFile = {
					...
					OnStartRunningUrl: function (aUrl) {
						cardbookLog.updateStatusProgressInformationWithDebug2("debug mode : start downloading attachment…");
					},
					OnStopRunningUrl: function (aUrl, aStatus) {
						if (aStatus == 0) {
							cardbookLog.updateStatusProgressInformationWithDebug2("debug mode : attachment successfully downloaded");
							var myTopic = "cardsImportedFromFile";
							var myActionId = cardbookActions.startAction(myTopic, [this.mFile.leafName]);
							cardbookSynchronization.loadFile(this.mFile, this.myDirPrefId, this.myDirPrefId, "WINDOW", "IMPORTFILE", myActionId);
