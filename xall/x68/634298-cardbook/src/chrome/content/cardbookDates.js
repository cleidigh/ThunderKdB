if ("undefined" == typeof(cardbookDates)) {
	...
	var cardbookDates = {
			...
			try {
				...
				if (myFieldValue == "") {
					...
					switch(myFieldValue.length) {
						...
						// dates
						default:
							var dateFormat = cardbookPreferences.getDateFormat(aCard.dirPrefId, aCard.version);
		...
		convertCardDate: function (aCard, aDirPrefName, aSourceDateFormat, aTargetDateFormat) {
			...
			for (var field of cardbookRepository.dateFields) {
				if (aCard[field] && aCard[field] != "") {
					...
					if (isDate != "WRONGDATE") {
						...
						cardbookUtils.formatStringForOutput("dateEntry1Wrong", [aDirPrefName, aCard.fn, myFieldValue, aSourceDateFormat], "Warning");
					}
				}
			}
			...
			var myEvents = cardbookUtils.getCardEvents(myNoteArray, aCard.others);
			if (myEvents.result.length != 0) {
				...
				cardbookUtils.addEventstoCard(aCard, myEvents.result, myPGNextNumber, aTargetDateFormat);
				eventsChanged = true;
			}
