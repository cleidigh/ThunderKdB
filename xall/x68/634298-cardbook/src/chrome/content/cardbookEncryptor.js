if ("undefined" == typeof(cardbookEncryptor)) {
	...
	var EXPORTED_SYMBOLS = ["cardbookEncryptor"];
	var cardbookEncryptor = {
		...
		storeKey: function (key, version) {
			...
			newLogin.init(this.getLoginName(version), null, cardbookRepository.strBundle.GetStringFromName("keyDescription"), "", key, "", "");
			var modified = false;
			try {
		...
		localizeLogin: function (version) {
			var correctLogins = Services.logins.findLogins(this.getLoginName(version), null, cardbookRepository.strBundle.GetStringFromName("keyDescription"));
			...
			var newLogin = Components.classes["@mozilla.org/login-manager/loginInfo;1"].createInstance(Components.interfaces.nsILoginInfo);
			newLogin.init(baseLogin.hostname, null, cardbookRepository.strBundle.GetStringFromName("keyDescription"), "", baseLogin.password, "", "");
		...
		saveCount: function() {
			cardbookPreferences.setStringPref("extensions.cardbook.localDataEncryption.counter", JSON.stringify(Array.from(this.counter)));
		},

		loadCount: function() {
			var counter = cardbookPreferences.getStringPref("extensions.cardbook.localDataEncryption.counter", "");
			if (counter) {
				try {
