	...
	var wdw_csvTranslator = {
				...
				for (var j = 0; j < cardbookRepository.allColumns[i].length; j++) {
					if (i != "arrayColumns" && i != "categories") {
						if (cardbookRepository.allColumns[i][j] == aField) {
							return ConversionHelper.i18n.getMessage(cardbookRepository.allColumns[i][j] + "Label");
						}
			...
			for (var i = 0; i < cardbookRepository.allColumns.arrayColumns.length; i++) {
				for (var k = 0; k < cardbookRepository.allColumns.arrayColumns[i][1].length; k++) {
					if (cardbookRepository.allColumns.arrayColumns[i][0] + "." + k + ".all" == aField) {
						return ConversionHelper.i18n.getMessage(cardbookRepository.allColumns.arrayColumns[i][1][k] + "Label");
					} else if (cardbookRepository.allColumns.arrayColumns[i][0] + "." + k + ".notype" == aField) {
						return ConversionHelper.i18n.getMessage(cardbookRepository.allColumns.arrayColumns[i][1][k] + "Label") + " (" + ConversionHelper.i18n.getMessage("importNoTypeLabel") + ")";
					}
				}
			}
			for (var i = 0; i < cardbookRepository.allColumns.arrayColumns.length; i++) {
				...
				for (var j = 0; j < myPrefTypes.length; j++) {
					for (var k = 0; k < cardbookRepository.allColumns.arrayColumns[i][1].length; k++) {
						if (cardbookRepository.allColumns.arrayColumns[i][0] + "." + k + "." + myPrefTypes[j][1] == aField) {
							return ConversionHelper.i18n.getMessage(cardbookRepository.allColumns.arrayColumns[i][1][k] + "Label") + " (" + myPrefTypes[j][0] + ")";
						}
					}
				}
			}
			if ("blank" == aField) {
				return ConversionHelper.i18n.getMessage(window.arguments[0].mode + "blankColumn");
			}
			return "";
		},
		...
		validateImportColumns: function () {
			if (wdw_csvTranslator.cardbookeditlists.foundColumns.length != wdw_csvTranslator.cardbookeditlists.addedColumns.length) {
				...
				var confirmMsg = ConversionHelper.i18n.getMessage("missingColumnsConfirmMessage");
		...
		startDrag: function (aEvent, aTreeChildren) {
			try {
				...
				cardbookRepository.cardbookLog.updateStatusProgressInformation("wdw_csvTranslator.startDrag error : " + e, "Error");
			}
		},
		...
		guess: function () {
			...
			if (!oneFound) {
				...
				for (var i = 0; i < wdw_csvTranslator.cardbookeditlists.foundColumns.length; i++) {
					...
					var found = false;
					for (var j = 0; j < wdw_csvTranslator.cardbookeditlists.availableColumns.length; j++) {
						var myTranslatedColumn = wdw_csvTranslator.getTranslatedField(wdw_csvTranslator.cardbookeditlists.availableColumns[j][0], "locale-US");
		...
		load: function () {
			...
			document.title = ConversionHelper.i18n.getMessage(window.arguments[0].mode + "MappingTitle");
			document.getElementById('availableColumnsGroupboxLabel').value = ConversionHelper.i18n.getMessage(window.arguments[0].mode + "availableColumnsGroupboxLabel");
			document.getElementById('addedColumnsGroupboxLabel').value = ConversionHelper.i18n.getMessage(window.arguments[0].mode + "addedColumnsGroupboxLabel");
			
			wdw_csvTranslator.cardbookeditlists.availableColumns = [];
			wdw_csvTranslator.cardbookeditlists.addedColumns = [];
			
			document.getElementById('fieldDelimiterLabel').value = ConversionHelper.i18n.getMessage("fieldDelimiterLabel");
			document.getElementById('includePrefLabel').value = ConversionHelper.i18n.getMessage("includePrefLabel");
			document.getElementById('lineHeaderLabel').value = ConversionHelper.i18n.getMessage("lineHeaderLabel");
			...
			if (window.arguments[0].mode == "choice") {
				...
				document.getElementById('foundColumnsGroupboxLabel').value = ConversionHelper.i18n.getMessage(window.arguments[0].mode + "foundColumnsGroupboxLabel");
				document.getElementById('includePrefLabel').hidden = true;
				document.getElementById('includePrefCheckBox').hidden = true;
				document.getElementById('lineHeaderCheckBox').setAttribute('checked', true);
				document.getElementById('fieldDelimiterTextBox').value = window.arguments[0].columnSeparator;
				wdw_csvTranslator.blankColumn = ConversionHelper.i18n.getMessage(window.arguments[0].mode + "blankColumn");
			...
			wdw_csvTranslator.cardbookeditlists.availableColumns = wdw_csvTranslator.cardbookeditlists.availableColumns.concat(cardbookRepository.cardbookUtils.getAllAvailableColumns(window.arguments[0].mode));
		...
		loadTemplateNext: function (aFile) {
			try {
				if (aFile) {
					cardbookRepository.cardbookSynchronization.getFileDataAsync(aFile.path, wdw_csvTranslator.loadTemplateNext2, {});
				}
			}
			catch (e) {
				cardbookRepository.cardbookLog.updateStatusProgressInformation("wdw_csvTranslator.loadTemplateNext error : " + e, "Error");
			}
		},
		...
		loadTemplateNext2: function (aContent, aParams) {
			try {
				...
				cardbookRepository.cardbookLog.updateStatusProgressInformation("wdw_csvTranslator.loadTemplateNext2 error : " + e, "Error");
			}
		},
		...
		saveTemplateNext: function (aFile) {
			try {
				...
				cardbookRepository.cardbookLog.updateStatusProgressInformation("wdw_csvTranslator.saveTemplateNext error : " + e, "Error");
			}
		},
