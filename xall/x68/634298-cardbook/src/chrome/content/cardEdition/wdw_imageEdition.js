	...
	var wdw_imageEdition = {
		...
		writeImageToFile: function (aFile, aDataValue, aExtension) {
			...
			NetUtil.asyncCopy(aDataValue, ostream, function (status) {
				if (Components.isSuccessCode(status)) {
					...
					cardbookLog.updateStatusProgressInformation("cardbookSynchronization.writeImageToFile error : filename : " + aFile.path, "Error");
				}
			});
		},
		...
		displayImageCard: function (aCard, aDisplayDefault) {
			if (aCard.photo.localURI) {
				...
				if (aDisplayDefault) {
					wdw_imageEdition.resizeImageCard("chrome://cardbook/skin/missing_photo_200_214.png", aDisplayDefault);
				} else {
					document.getElementById('imageBox').setAttribute('hidden', 'true');
				}
			}
		...
		pasteImageCard: function () {
			try {
				...
				cardbookLog.updateStatusProgressInformation("wdw_imageEdition.pasteImageCard error : " + e, "Error");
			}
		},
		...
		copyImageCard: function () {
			try {
				...
				cardbookLog.updateStatusProgressInformation("wdw_imageEdition.copyImageCard error : " + e, "Error");
			}
		},
		...
		copyImageLocationCard: function () {
			try {
				...
				cardbookLog.updateStatusProgressInformation("wdw_imageEdition.copyImageLocationCard error : " + e, "Error");
			}
		},
