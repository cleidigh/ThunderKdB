...
var gc_functions = {
    ...
    _c_mc_mot_composé_1: function (sSentence, sSentence0, m, dTokenPos, sCountry, bCondMemo) {
        return ! m[1].gl_isDigit() && ! m[2].gl_isDigit() && ! morph(dTokenPos, [m.start[0], m[0]], ":") && ! morph(dTokenPos, [m.start[2], m[2]], ":G") && gc_engine.oSpellChecker.isValid(m[1]+m[2]);
    },
    _c_mc_mot_composé_2: function (sSentence, sSentence0, m, dTokenPos, sCountry, bCondMemo) {
        return m[2] != "là" && ! re.search("(?i)^(?:ex|mi|quasi|semi|non|demi|pro|anti|multi|pseudo|proto|extra)$", m[1]) && ! m[1].gl_isDigit() && ! m[2].gl_isDigit() && ! morph(dTokenPos, [m.start[2], m[2]], ":G") && ! morph(dTokenPos, [m.start[0], m[0]], ":") && ! gc_engine.oSpellChecker.isValid(m[1]+m[2]);
    },
    ...
    _g_cond_ocr_28: function (lToken, nTokenOffset, nLastToken, sCountry, bCondMemo, dTags, sSentence, sSentence0) {
        return gc_engine.oSpellChecker.isValid(lToken[nTokenOffset+1]["sValue"].slice(1));
    },
    ...
    _g_cond_ocr_30: function (lToken, nTokenOffset, nLastToken, sCountry, bCondMemo, dTags, sSentence, sSentence0) {
        return look(sSentence.slice(0,lToken[1+nTokenOffset]["nStart"]), "[a-zA-Zà-öÀ-Ö0-9_ø-ÿØ-ßĀ-ʯﬁ-ﬆᴀ-ᶿ]") && g_morph(lToken[nTokenOffset+1], ":", ":M") && gc_engine.oSpellChecker.isValid(lToken[nTokenOffset+1]["sValue"].slice(1));
    },
