...
var EXPORTED_SYMBOLS = ['executeMindrInWindow', 'getMessengerWindow'];
...
var { getFolder, isMindrInboxZero } = ChromeUtils.import(
...
async function executeBasicOperationsForHdr(mindr, window) {
    return new Promise((resolve, reject) => {
        ...
        const messages = mailmindrSearch.getMessageHdrByMessageId(
            mindr.mailguid
        );
        ...
        try {
            for (const msg of messages) {
                const headers = Components.classes[
                    '@mozilla.org/array;1'
                ...
                if (mindr.doMarkFlag) {
                    ...
                    msg.folder.markMessagesFlagged(
                        toXPCOMArray([msg.hdr], Ci.nsIMutableArray),
                        true
                    );
                }
...
async function executeMoveOrCopy(mindr, window) {
    return new Promise((resolve, reject) => {
        ...
        if (messages == null || messages.length == 0) {
            logger.error(
                `${_logMarker} - mindr wants to be executed - but the mail is lost. I don\'t know what to do. Will try again later.`
            );
            return false;
        }
        ...
        for (const msg of messages) {
            ...
            if (!targetURI && isMindrInboxZero(mindr)) {
                // 
                moveOrCopy = 1;
            ...
            if (
                ...
                logger.log(
                    `no ove or copy operation queued for '${mindr.mailmindrGuid}' w/ mailguid '${mindr.mailguid}'`
                );
            ...
            try {
                ...
                const copyListener = {
                    ...
                    OnStopCopy: function(aStatus) {
                        if (Components.isSuccessCode(aStatus)) {
                            logger.log(
                                'copy stopped: releasing msg databases.'
                            );
                            // 
                            // 
                            // 
                            ...
                            // 
                            logger.log(
                                `${mindr.mailmindrGuid} - copied successfully. checking for new mindrs to execute.`
                            );
                            finishMindrAndSetToDone(mindr);
                            // 
                            ...
                            return reject(
                                new Error(
                                    `Copy failed for mindr ${mindr.mailmindrGuid}`
                                )
                            );
                        }
                    }
                ...
                const copyService = Components.classes[
                ...
                if (moveOrCopy == 1) {
                    ...
                    if (folderURI != sourceURI) {
                        ...
                        logger.log(
                            `${_logMarker} - move failed, URIs are equal`
                        );
                    }
                } // -- if move
            ...
            logger.log(
                `${_logMarker} -  executed, db status: ${msg.folder.msgDatabase}`
            );
...
async function executeMindrInWindow(mindr, deleteMindr) {
    // 
    try {
        logger.log(
            `START exec mindr: ${mindr.mailmindrGuid} w/ id: '${mindr.mailguid}'`
        );
        ...
        if (mindr.performed) {
            if (mindrDoNotShowDialog) {
                logger.log(
                    'mindr ALREADY shown and PERFORMED (will be deleted): ' +
                        mindr.mailmindrGuid
                );
        ...
        const basicOperationsSuccess = await executeBasicOperationsForHdr(
            mindr,
            messenger
        );
...
function finishMindrAndSetToDone(mindr) {
    logger.log(
        'DONE :: all messages for mindr ' + mindr.mailmindrGuid + ' performed.'
    );
