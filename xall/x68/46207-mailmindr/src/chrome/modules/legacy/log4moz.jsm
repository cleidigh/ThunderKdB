...
const PERMS_FILE = 0o0644;
const PERMS_DIRECTORY = 0o0755;
...
let Log4Moz = {
    ...
    // 
    // 
    enumerateProperties: function Log4Moz_enumerateProps(
        aObject,
        ...
        for (p in aObject) {
            try {
                if (
                    aExcludeComplexTypes &&
                    (typeof aObject[p] == 'object' ||
                        typeof aObject[p] == 'function')
                )
                    continue;
...
function Logger(name, repository) {
    if (!repository) repository = Log4Moz.repository;
    this._name = name;
    this.children = [];
    this.ownAppenders = [];
    this.appenders = [];
...
Logger.prototype = {
    ...
    updateAppenders: function updateAppenders() {
        if (this._parent) {
            let notOwnAppenders = this._parent.appenders.filter(function(
                appender
            ) {
    ...
    log: function Logger_log(level, string) {
        ...
        for (let i = 0; i < appenders.length; i++) {
            ...
            if (!message) message = new LogMessage(this._name, level, string);
...
LoggerRepository.prototype = {
    ...
    _updateParents: function LogRep__updateParents(name) {
        ...
        // 
        if (!parent) this._loggers[name].parent = this.rootLogger;
        else this._loggers[name].parent = this._loggers[parent];
    ...
    getLogger: function LogRep_getLogger(name) {
        if (name in this._loggers) return this._loggers[name];
...
function RotatingFileAppender(file, formatter, maxSize, maxBackups) {
    if (maxSize === undefined) maxSize = ONE_MEGABYTE * 2;

    if (maxBackups === undefined) maxBackups = 0;
...
RotatingFileAppender.prototype = {
    ...
    rotateLogs: function RFApp_rotateLogs() {
        if (this._file.exists() && this._file.fileSize < this._maxSize) return;
        ...
        for (let i = this.maxBackups - 1; i > 0; i--) {
            ...
            if (backup.exists())
                backup.moveTo(
                    this._file.parent,
                    this._file.leafName + '.' + (i + 1)
                );
        }

        let cur = this._file.clone();
        if (cur.exists()) cur.moveTo(cur.parent, cur.leafName + '.1');
