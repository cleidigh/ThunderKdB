...
var {
    ...
    mailmindrLogger: MailmindrLogger
} = ChromeUtils.import('chrome://mailmindr/content/modules/legacy/logger.jsm');
...
class mailmindrStorageBase {
    ...
    _createDatabase(aDBService, aDBFile) {
        this._logger.log(`!!> opening database file: ${aDBFile}`);
    ...
    _createTables(aDBConnection) {
        for (let tableName of this._schema.tableNames) {
            ...
            aDBConnection.createTable(
                tableName,
                this._schema.tables[tableName]
            );
            this._logger.log('!!>   [done]');
        }
    }
    ...
    _ensureDatabase() {
        return new Promise((resolve, reject) => {
            try {
                ...
                var dbService = Cc['@mozilla.org/storage/service;1'].getService(
                ...
                if (!dbFile.exists()) {
                    this._logger.log(
                        '!!> mailmindr.sqlite not found. initiating new database file.'
                    );
    ...
    _runDatabaseUpdate(updateFromVersion, updateToVersion) {
        ...
        log(
            'run database update from version ' +
                updateFromVersion +
                ' > to > ' +
                updateToVersion
        );
        ...
        for (let version in this._schema.updates) {
            if (version <= updateFromVersion || version > updateToVersion) {
                log('skip version ' + version);

                if (version <= updateFromVersion) {
                    log(' upd ' + version + ' < ' + updateFromVersion);
                }

                if (version > updateToVersion) {
                    log(' upd ' + version + ' > ' + updateToVersion);
                }
                continue;
            }
        ...
        for (let version of versions) {
            log('run update for ' + version);
            try {
                let commands = this._schema.updates[version];

                if (commands.length == 0) {
                    log('version ' + version + ' has no updates');
    ...
    _prepareStatements() {
        if (this._db) {
            ...
            this._commands.getAllActiveMindrs = this._createStatement(
                queryMindr + 'WHERE isEnabled = 1 ORDER BY remindat ASC'
            );

            this._commands.getMindrByGuid = this._createStatement(
                queryMindr + 'WHERE mindr.mailmindrGuid = :mailmindrGuid'
            );
            ...
            this._logger.error(
                'houston, we have a problem: the database is gone.'
            );
        }
    }
    ...
    loadMindrs() {
        ...
        if (!this._initialized) {
            this._logger.warn('currently _not_ initialized, will return []');
            return [];
        }
        ...
        this._logger.log(
            `loadMindrs loaded ${mindrBuffer &&
                mindrBuffer.length} mindrs from storage`
        );
    ...
    saveMindr(mindr) {
        ...
        const result = this._execute(query, fields, mindr);
        mindr.id = this._getLastInsertId();

        const success = result && this.saveMindrDetails(mindr);

        if (success) {
            Services.obs.notifyObservers(
                null,
                'mailmindr-storemindr-success',
                JSON.stringify(mindr)
            );
        }
    ...
    updateMindr(mindr) {
        ...
        const queryNotes = this._commands.updateMindrDetails;
        const fieldNotes = ['note', 'mailmindrGuid'];
        const obj = {
        ...
        const success =
            this._execute(query, fields, mindr) &&
    ...
    deleteMindrDetails(mindr) {
        this._execute(
            this._commands.deleteMindrDetails,
            ['mailmindrGuid'],
            mindr
        );
    }
    ...
    getCurrentDatabaseVersion() {
        ...
        if (!this._db.tableExists('settings')) {
            this._logger.log(
                'Database version cannot be read, table is missing'
            );
            return defaultValue;
        }

        let query = this._createStatement(
    ...
    setPreference(key, value) {
        ...
        if (typeof previousValue == 'undefined') {
            this._logger.log(`initialize preference: ${key}`);
            query = this._commands.initPreference;
        }

        return this._execute(query, ['key', 'value'], {
            key: key,
            value: value
        });
    }
    ...
    async initialize() {
        ...
        const updateDatabase = () =>
            new Promise((resolve, reject) => {
                ...
                try {
                    AddonManager.getAddonByID('mailmindr@arndissler.net').then(
                        addon => {
                            scope._logger.log(
                                '** sysinfo ** running mailmindr v' +
                                    addon.version
                            );
                            ...
                            if (currentVersion < addon.version) {
                                // 
                                scope._logger.log(
                                    'run update from version ' +
                                        currentVersion +
                                        ' to ' +
                                        addon.version
                                );
                                scope._runDatabaseUpdate(
                                    currentVersion,
                                    addon.version
                                );
                            } else {
                            ...
                            const result = scope.safeCall(
                                scope,
                                scope._prepareStatements,
                                scope._logger
                            );
        ...
        const result = this._initialized || false;
    ...
    setUpdateFlag() {
        let prefs = Components.classes['@mozilla.org/preferences-service;1']
            ...
            .getBranch('extensions.mailmindr.');
...
async function getMailmindrStorage() {
        ...
        pendingInitializer = new Promise((resolve, reject) => {
            ...
            if (mailmindrStorage) {
                ...
                try {
                    ...
                    mailmindrStorage.initialize().then(success => {
                        if (success) {
                            log.log(`storage initialized, resolve.`);
                            resolve(mailmindrStorage);
                        } else {
                            log.error(`storage cannot be initialized`);
                            reject();
                        }
                    });
                } catch (e) {
                    log.error(
                        `storage canot be initialized, rejected due to ${e}`
                    );
                    reject(e);
                }
            }
