...
mailmindr.dialogs.setmindr = {
    ...
    Initialize: function() {
        ...
        if (!this._initialized) {
            try {
                ...
                document
                    .getElementById('mailmindrDatePickerWrapper')
                    .appendChild(dtp);
                this._logger.log('datepicker created.');

                this._elements.textMailSubject = document.getElementById(
                    'mailmindrMailSubject'
                );
                // 

                this._elements.textMailSender = document.getElementById(
                    'mailmindrMailSender'
                );

                this._elements.timespans = document.getElementById(
                    'mailmindrTimespans'
                );
                //this._elements.datePicker = document.getElementById("mailmindrDatePicker");
                this._elements.datePicker = dtp;
                this._elements.timePicker = document.getElementById(
                    'mailmindrTimePicker'
                );

                this._logger.log(
                    'check if datepicker and timepicker are created'
                );
                this._logger.log(
                    `datetpicker (should be XUL element: ${this._elements.datePicker})`
                );
                this._logger.log(
                    `timepicker (should be XUL element: ${this._elements.timePicker})`
                );
                this._logger.log('check for datepicker and timepicker done.');

                this._elements.actionPicker = document.getElementById(
                    'mailmindrActionPicker'
                );
                this._elements.buttonAccept = document.documentElement.getButton(
                    'accept'
                );
                this._elements.doSetReminder = document.getElementById(
                    'mailmindrDoSetReminder'
                );
                this._elements.noteTextBox = document.getElementById(
                    'mailmindrNotes'
                );
                ...
                this._logger.log(
                    'set mindr :: type:     ' + typeof this._data.mindr
                );
                this._logger.log(
                    'set mindr :: timespan: ' + this._data.selectedTimespan
                );
                this._logger.log(
                    'set mindr :: action:   ' + this._data.selectedAction
                );
                ...
                // 
                if (!this._data.selectedTimespan) {
                    ...
                    if (!this._hasMindr()) {
                        ...
                        this._elements.doSetReminder.setAttribute(
                            'checked',
                            MailmindrPreferences.getBoolPref(
                        ...
                        //
                        this._logger.log(
                            'mindr IS ALREADY SET (modifying the old one)'
                        );
                        this._logger.log(
                            'doShowDialog: ' + this._data.mindr.doShowDialog
                        );
                        this._controls.actions.Action = this._data.mindr;
                        this._elements.timespans.value = '-1;-1;-1;false';
                        this._controls.timespans.setDateTime(
                            new Date(this._data.mindr.remindat)
                        );
                        this._elements.doSetReminder.setAttribute(
                            'checked',
                    ...
                    if (!this._hasMindr()) {
                        ...
                        const checked = MailmindrPreferences.getBoolPref(
                            'common.showAlertDialog'
                        );
                        this._elements.doSetReminder.setAttribute(
                            'checked',
                ...
                let key =
                    mailmindrCore.getAccountKeyFromFolder(
                        this._data.selectedMail.folder
                    ) || 'mailmindr:global';
                let folder = mailmindrCore.getInboxZeroFolder(key);
                this._logger.log(
                    `account key: ${key} which is mapped to folder: ${folder}`
                );

                if (folder === '' && key !== 'mailmindr:global') {
                    // 
                    this._logger.log(
                        `checking for global fallback 'mailmindr:global'`
                    );
                    folder = mailmindrCore.getInboxZeroFolder(
                        'mailmindr:global'
                    );
                    this._logger.log(
                        `~> fallback for 'mailmindr:global' is ${folder}`
                    );
                }
                ...
                let notifyMsg = document.getElementById(
                    'mailmindrInboxZeroActiveNotificationMessage'
                );
                if (this.enableInboxZero) {
                    let messageIdentifier =
                        'mailmindr.dialog.setmindr.inboxzero.notificationmessage';
                    let information = mailmindrI18n.getString(
                        messageIdentifier
                    );
                    ...
                    let localJson = [
                        [
                            ...
                            mailmindrI18n.getString(
                                'mailmindr.dialog.setmindr.inboxzero.notificationmessage.label'
                            ),
                            ' '
                        ],
                        [
                            ...
                            {},
                            mailmindrI18n.getString(
                                'mailmindr.dialog.setmindr.inboxzero.notificationmessage.link_before'
                            ),
                            ' '
                        ],
                        [
                        ...
                        [
                            ...
                            {},
                            mailmindrI18n.getString(
                                'mailmindr.dialog.setmindr.inboxzero.notificationmessage.link_after'
                            )
                        ]
                    ];

                    let localNodes = {};
                    let localParsedElement = mailmindrKernel.kernel.modules.common.jsonToDom(
                        localJson,
                        document,
                        localNodes
                    );
    ...
    setEventListeners: function() {
        ...
        const timespanPicker = document.getElementById('mailmindrTimespans');
        timespanPicker.addEventListener('select', () =>
            this.onSelectTimespan()
        );

        this._controls.actions.addEventListener('selectAction', function(
            action
        ) {
...
--
	...
	*/
     ...
     */
    onSelectTimespan: function() {
        const timeSpanCtrl = this._controls.timespans;
        if (timeSpanCtrl) {
            const pickedTime = timeSpanCtrl.dateValue;
            const disabled = pickedTime + 60 * 1000 < Date.now();

            this._elements.buttonAccept.disabled = disabled;
        }
    },
    ...
    onSelectAction: function(target, action) {
        ...
        // 
        const disabled = action.doMoveOrCopy != 0 && this.enableInboxZero;
        target._elements.buttonAccept.disabled = disabled;
    },

    /**
     * onDialogAccept
     * @returns boolean Returns true when mindr can be stored, false otherwise
     * */
    onDialogAccept: async function() {
        try {
            const mindr = await this._createMindr();
            ...
            await this._persistMindr(mindr, () => {
                // 
                // 
                // 
     ...
     * @returns mindr
     */
    _createMindr: async function() {
        return new Promise((resolve, reject) => {
            let mindr = null;
            if (this._hasMindr()) {
                // 
                const dummy = this._data.mindr;
                const action = this._controls.actions.Action;
                ...
                // 
                mindr = mailmindrCore.createMindrWithAction(
                    this._controls.actions.Action
                );
            ...
            const dateValue = this._controls.timespans.dateValue;
            mindr.remindat = dateValue.getTime();
    ...
    _persistMindr: async function(mindr, onSuccess, onFailure) {
        return new Promise((resolve, reject) => {
            if (this._hasMindr()) {
                ...
                if (this.enableInboxZero) {
                    const { selectedMail } = this._data;
                    if (selectedMail) {
                        const { folder } = selectedMail;
                        mindr.originFolderURI = folder && folder.URI;
                    }
                }
                ...
                if (mailmindrStorage.saveMindr(mindr)) {
                    ...
                    if (!this.enableInboxZero) {
                        ...
                        try {
                            this._logger.log(
                                `Inbox Zero is ENABLED, w/ data: ${this._data.selectedMail} and ${this.inboxZeroLaterFolder}`
                            );
                            ...
                            this._logger.log(
                                'setMindr._persistMindr > moving the mail failed, because: ' +
                                    exc
                            );
    ...
    _storeSelectedAction: function() {
        try {
            const actionPicker = document.getElementById(
                'mailmindrActionPicker'
            );
            const rawAction = actionPicker && actionPicker.value;
            if (rawAction) {
                MailmindrPreferences.setStringPref(
                    'custom.lastSelectedAction',
                    rawAction
                );
            }
        } catch (ex) {
            this._logger.error(`cannot store selected action: ${ex}`);
        }
    },

    _hasMindr: function() {
        return (
            typeof this._data.mindr != 'undefined' && this._data.mindr != null
        );
    },

    moveMailToInboxZeroLaterFolder: function(
        mindr,
        message,
        targetFolderURI,
        onSuccess
        ...
        if (sourceURI == folderURI) {
            this._logger.log(
                'Destination and source folder is equal - cancel copy.'
            );
            onSuccess();
            return;
        }

        let headers = Components.classes['@mozilla.org/array;1'].createInstance(
            Components.interfaces.nsIMutableArray
        );

        headers.appendElement(message, false);

        let copyService = Components.classes[
        ...
        // 
        var threePaneWindow = mailmindrKernel.kernel.modules.common.getWindow(
            'mail:3pane'
        ).msgWindow;
        if (!threePaneWindow) {
