...
var mailmindrPrefPaneCommon = {
    ...
    initialize: function() {
        ...
        try {
            ...
            this._elements.tagSelector = document.getElementById(
                'mailmindrOnTagSelector'
            );
            this._elements.tagTimespanSelector = document.getElementById(
                'mailmindrOnTagTimespanSelector'
            );

            this._elements.defaultTimespanSelector = document.getElementById(
                'mailmindrDefaultTimespanSelector'
            );
            this._elements.defaultActionSelector = document.getElementById(
                'mailmindrDefaultActionSelector'
            );

            this._elements.targetFolderIncoming = document.getElementById(
                'mailmindrTargetFolderIncoming'
            );
            this._elements.targetFolderSent = document.getElementById(
                'mailmindrTargetFolderSent'
            );
            this._elements.inboxZeroLaterFolder = document.getElementById(
                'mailmindrInboxZeroLaterFolder'
            );
            ...
            this._controls.defaultAction = new mailmindr.controls.ActionPicker(
                this._elements.defaultActionSelector,
                {
                    ...
                    doNothingLabel: mailmindrI18n.getString(
                        'mailmindr.utils.core.actiontemplate.picklastusedaction'
                    )
                }
            );
    ...
    refresh: function() {
        ...
        // 
        mailmindrCommon.fillTags(
            this._elements.tagSelector,
            0,
            MailmindrPreferences.getStringPref(
                'common.setMindrOnTagSelectedTag'
            )
        );
        ...
        // 
        let folderIncoming = MailmindrPreferences.getStringPref(
            'common.targetFolderIncoming'
        );
        let folderSent = MailmindrPreferences.getStringPref(
            'common.targetFolderSent'
        );
        let inboxZeroLaterFolder = MailmindrPreferences.getStringPref(
            'common.inboxZeroLaterFolder'
        );
        let onTagSetTimespan = MailmindrPreferences.getStringPref(
            'common.setMindrOnTagSelectedTimespan'
        );
        let defaultTimespan = MailmindrPreferences.getStringPref(
            'common.setMindrDefaultSelectedTimespan'
        );

        let serializedDefaultAction = MailmindrPreferences.getStringPref(
        ...
        if (serializedDefaultAction.length > 2) {
            try {
                ...
                this._logger.error(
                    `cannot set defaultAction: '${serializedDefaultAction}'`
                );
        ...
        for (let timespan of timespans) {
            ...
            this._elements.defaultTimespanSelector.appendItem(
                timespan.text,
                key
            );
        }
        ...
        for (let element of [
            this._elements.tagTimespanSelector,
            this._elements.defaultTimespanSelector
        ]) {
    ...
    get elements() {
        return (
            document.querySelectorAll(
                '#mailmindrPreferences [data-preference]'
            ) || []
        );
    },
    ...
    getPreferenceMapping: function(preference) {
        const mapping =
            mailmindrPreferencesDialog &&
            mailmindrPreferencesDialog.dataMapping &&
            mailmindrPreferencesDialog.dataMapping[preference];
        if (!Boolean(mapping)) {
            return null;
        }
            ...
            if (prefName.indexOf('common.action.default') >= 0) {
                ...
                if (value) {
                    try {
                        ...
                        this._logger.error(
                            `loadPreferences: cannot set default action: ${value}`
                        );
                    }
                }
            } else if (value !== undefined) {
                this._logger.log(
                    `loaded >> ${prefName} :: set to :: '${value}' using element '${element}'`
                );
    ...
    savePreferences: function() {
        this.elements.forEach(element => {
            ...
            this._logger.log(`get pref value :: ${prefName}: '${value}'`);
