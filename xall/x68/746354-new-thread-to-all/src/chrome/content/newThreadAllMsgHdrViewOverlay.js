...
function getIdentityForHeader(hdr, type)
{
  ...
  if (!identity)
    identity = MailUtils.getBestIdentity(accountManager.allIdentities, hintForIdentity);
...
function parse(aMimeLine) {
  ...
  return [names.value, emails.value, fullNames.value];
}
...
//alert(msgHdr.author);
  let [recipients, recipientsEmailAddresses, fullNames] = parse(msgHdr.recipients);
...
// alert(FromMe);
let newRecipients= "";
let myEmail= newThreadIdentity.email.toLowerCase();
let recAddr="";
let i=0;
if (ToMe)
{
   for (i = 0;i<recipientsEmailAddresses.length;i++ )
   {
      recAddr= recipientsEmailAddresses[i];
      if (!(recAddr.includes(myEmail) )  )
       {
       if (i>0) newRecipients=  newRecipients+ ",";
       newRecipients=  newRecipients+ fullNames[i];
      }
   }
}
   else  newRecipients = msgHdr.recipients;
...
if (FromMe) compFields.to = msgHdr.recipients; else compFields.to = msgHdr.author +  "," + newRecipients ;
