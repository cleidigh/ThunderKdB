...
function NostalgyFolderSelectionBox(box) {
  var cmd = box.getAttribute("nostalgyfolderbox"); //? only for edit.rule and keys??
  if (cmd) {
    //  box.setAttribute("ontextentered",cmd); //? ontextentered doesn't exist for html:input?
    //  box.setAttribute("ontextcommand",cmd); //? ontextcommand doesn't exist for html:input?
    //box.setAttribute("maxrows", "15"); //? use what is selected in xhtml
  ...
  box.onkeypress = function (event) {
    if (event.key == "Tab" && box.getAttribute("normaltab") != "true") {
      ...
      if (event.key == "Enter" && box.getAttribute("normaltab") != "true") {
        ...
        //       if (event.id = "underselect") onNostalgyKeyPressUnder(); //opens unwanted alert
        event.preventDefault();
