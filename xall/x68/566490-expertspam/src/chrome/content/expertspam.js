...
expertspam.extend({
	...
	translate: function(id) {
		if(!this.stringbundle) this.stringbundle = this.getClassService('intl/stringbundle', 'nsIStringBundleService').createBundle('chrome://expertspam/locale/expertspam.properties');
		try {
			return this.stringbundle.GetStringFromName('expertspam.' + id);
		}catch(e) {
			return '';
		}
	},
	...
	pref: function(id, getter, value) {
		...
		if(id[0] == '.') id = 'extensions.expertspam' + id;
		try {
...
expertspam.extend({
	...
	isReportedAs: function(what, hdr) {
		var r = hdr ? hdr.getStringProperty('expertspam-reported-as-' + what) : '';
		return (r == '1');
	},
	...
	viewedMsgOnBeforeShowHeaderPane: function () {
		...
		this.showHide('expertspam_button_spamham', showInToolbar);
		...
		if(isSpam) {
			if(this.isReportedAs('ham', selectedMessage)) {
				this.setTrigger(['expertspam_button_spamham', 'expertspam_menu_spamham'], 'reported.ham', '');
			}else{
				this.setTrigger(['expertspam_button_spamham', 'expertspam_menu_spamham'], 'report.ham', 'expertspam_cmd_ham');
			}
		}else{
			if(this.isReportedAs('spam', selectedMessage)) {
				this.setTrigger(['expertspam_button_spamham', 'expertspam_menu_spamham'], 'reported.spam', '');
			}else{
				this.setTrigger(['expertspam_button_spamham', 'expertspam_menu_spamham'], 'report.spam', 'expertspam_cmd_spam');
			}
		}
	...
	initialize: function() {
		...
		if(ctn && (installed != this.version) && set.indexOf('expertspam_button_spamham') < 0) {
			set.push('expertspam_button_spamham');
        ...
        addEventListener('contextmenu', function() {
            ...
            if (!showInMenu || selectedMessage === null) {
                rso.showHide('expertspam_menu_spamham', false);
                rso.showHide('expertspam_separator', false);
                return;
            }
            ...
            rso.showHide('expertspam_menu_spamham', showInMenu);
            rso.showHide('expertspam_separator', showInMenu);
            if(nothingToReport) {
                rso.setTrigger(['expertspam_button_spamham', 'expertspam_menu_spamham'], 'reported.spam', '');
            } else {
                rso.setTrigger(['expertspam_button_spamham', 'expertspam_menu_spamham'], 'report.spam', 'expertspam_cmd_spam');
            }
        });
...
expertspam.extend({
	...
	sendReportToServer: function(s, what) {
		...
		try {
			req.open('POST', "http://spamreport.spamrl.com/spamreport.php", true);
			req.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
			req.setRequestHeader('X-Reported-Via', "Expertspam/" + this.version + " (Thunderbird)" );
    ...
    process: function(what) {
        ...
        if (this.reportedMessagesCount == messages.length) {
            messages.forEach(function(message) {
                message.setStringProperty('expertspam-reported-as-' + what, '1');
            });

            var target = 'spamham';
            this.setTrigger(['expertspam_button_' + target, 'expertspam_menu_' + target], 'reported.' + what, '');
...
expertspam.extend({
	switchReportMode: function(what, mode) {
		if(typeof(what) != 'string') {
			var mode = what.value;
			var what = what.parentNode.parentNode.id.replace(/^expertspam_/, '');
		}
		var el = document.getElementById('expertspam_' + what + '_reportmode_' + mode);
		if(!el) return;
