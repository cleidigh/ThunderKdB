...
const CLASS_ID = Components.ID('bad3762e-bf92-444a-a9cf-e2409ea205d6');
const CLASS_NAME = "REST AutoComplete User";
const CONTRACT_ID = '@mozilla.org/autocomplete/search;1?name=rest-autocomplete-user';
...
ProviderAutoCompleteSearch.prototype = {
		...
		startSearch: function(searchString, searchParam, previousResult, listener) {
			...
			//consoleService.logStringMessage("rest-autocomplete-user:startSearch() searchString " + searchString);
			//get preferences
			var prefs = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService);
			prefs = prefs.getBranch("extensions.createjiraissue.");
			var jiraurl = prefs.getCharPref("jiraurl");
			//consoleService.logStringMessage("rest-autocomplete-user:startSearch() jiraurl " + jiraurl);
			var username = null;
			var password = null;
			...
			var url = jiraurl + "/rest/api/latest/user/search?username=" + lowerSearchString;
			//consoleService.logStringMessage("rest-autocomplete-user:startSearch() url " + url);
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4) {
					if ( xmlhttp.status == 200) {
						...
						//consoleService.logStringMessage("rest-autocomplete-user:startSearch() results " + results);
