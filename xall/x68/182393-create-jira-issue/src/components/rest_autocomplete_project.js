...
ProviderAutoCompleteSearch.prototype = {
		...
		startSearch: function(searchString, searchParam, previousResult, listener) {
			...
			//consoleService.logStringMessage("rest-autocomplete-project:startSearch() searchString " + searchString);
			//get preferences
			var prefs = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService);
			prefs = prefs.getBranch("extensions.createjiraissue.");
			var jiraurl = prefs.getCharPref("jiraurl");
			//consoleService.logStringMessage("rest-autocomplete-project:startSearch() jiraurl " + jiraurl);
			var username = null;
			var password = null;
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4) {
					if ( xmlhttp.status == 200) {
						...
						//consoleService.logStringMessage("rest-autocomplete-project:startSearch() results " + results);
						var filteredResults = new Array();
						var filteredComments = new Array();
						results = results.results;
						for ( var i = 0; i < results.length; i++ ) {
							 // we cannot use value, since we need the key !
							var raw = results[i].displayName;
							//consoleService.logStringMessage("rest-autocomplete-project:startSearch() raw result " + raw);
							var result = raw.replace(/<\/?[^>]+(>|$)/g,""); // taken from stackoverflow
							//consoleService.logStringMessage("rest-autocomplete-project:startSearch() post result " + result);
