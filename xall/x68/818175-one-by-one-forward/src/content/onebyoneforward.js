    ...
    let progressListener = {
      onStateChange: function (aWebProgress, aRequest, aStateFlags, aStatus) {
        ...
        if(aStateFlags & theWindow.Components.interfaces.nsIWebProgressListener.STATE_STOP) {
          ...
          if(aStatus) {
            callingWindowData.log.value += 'Failed: ' + JSON.stringify([aStateFlags, aStatus]) + '\n';
          }
          else {
            callingWindowData.log.value += 'Sent.\n';
          }
          onComplete();
        }
      },
      ...
      QueryInterface: theWindow.ChromeUtils.generateQI([
  ...
  OneByOneForward = {
    launch: function () {
      ...
      if(selectedMessages.length > 0) {
        let subWindow = theWindow.open('chrome://onebyoneforward/content/onebyoneforwardform.xul', '_blank', 'chrome,centerscreen,width=600,height=340');
        let id;
    ...
    sendSelected: function (callingWindow) {
      ...
      callingWindowData.submitButton = callingWindow.document.getElementById('onebyoneforward-submit');
      ...
      let sendTimer = function () {
        ...
        if(i < callingWindowData.selectedMessages.length) {
          ...
          send(
            ...
            function () {
              ...
              if(i == callingWindowData.selectedMessages.length) {
                ...
                callingWindow.setTimeout(sendTimer, 100);
              }
            }
          );
        }
          ...
          callingWindowData.inProgress = false;
          callingWindowData.submitButton.setAttribute('label', 'Close');
          callingWindowData.submitButton.setAttribute('oncommand', 'window.close()');
          callingWindowData.submitButton.disabled = false;
          callingWindowData.log.value += 'Done.\n';
          callingWindow.focus();
        }
      };
      sendTimer();
