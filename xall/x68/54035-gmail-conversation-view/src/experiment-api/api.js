...
const conversationModules = ["chrome://conversations/content/modules/plugins/dkimVerifier.js", "chrome://conversations/content/modules/plugins/enigmail.js", // Don't unload these until we can find a way of unloading the attribute
...
"chrome://conversations/content/modules/plugins/lightning.js", "chrome://conversations/content/modules/assistant.js", "chrome://conversations/content/modules/browserSim.js", "chrome://conversations/content/modules/contact.js", "chrome://conversations/content/modules/conversation.js", "chrome://conversations/content/modules/hook.js", "chrome://conversations/content/modules/message.js", "chrome://conversations/content/modules/misc.js", "chrome://conversations/content/modules/prefs.js"];
const kAllowRemoteContent = 2; // Note: we must not use any modules until after initialization of prefs,
  ...
  return {
    ...
    onDataAvailable(aRequest, aInputStream, aOffset, aCount) {
      if (this._stream == null) {
        this._stream = Cc["@mozilla.org/binaryinputstream;1"].createInstance(Ci.nsIBinaryInputStream);
...
function monkeyPatchWindow(win, windowId) {
  Log.debug("monkey-patching..."); // Insert our own global Conversations object
  ...
  ChromeUtils.import("chrome://conversations/content/modules/plugins/enigmail.js");
  ChromeUtils.import("chrome://conversations/content/modules/plugins/lightning.js");
  ChromeUtils.import("chrome://conversations/content/modules/plugins/dkimVerifier.js");
...
function getAttachmentInfo(win, msgUri, attachment) {
  const attInfo = new win.AttachmentInfo(attachment.contentType, attachment.url, attachment.name, msgUri, attachment.isExternal);
...
var conversations = class extends ExtensionCommon.ExtensionAPI {
  onStartup() {
    ...
    this.chromeHandle = aomStartup.registerChrome(manifestURI, [["content", "conversations", "content/"]]);
  }
    ...
    return {
      conversations: {
        async setPref(name, value) {
          ...
          if (name == "finishedStartup") {
            ...
            try {
              ...
              apiWindowObserver = new ApiWindowObserver(windowManager, monkeyPatchWindow);
        ...
        async getLegacyStorageData() {
          const path = OS.Path.join(OS.Constants.Path.profileDir, FILE_SIMPLE_STORAGE);
          ...
          const exists = await dbConnection.tableExists(SIMPLE_STORAGE_TABLE_NAME);

          if (!exists) {
            return [];
          }

          let rows = await dbConnection.execute(`SELECT key, value FROM ${SIMPLE_STORAGE_TABLE_NAME}`);
        ...
        async getAttachmentBody(id, partName) {
          ...
          return new Promise((resolve, reject) => {
            MsgHdrToMimeMessage(msgHdr, this, (mimeHdr, aMimeMsg) => {
              ...
              tmpChannel.asyncOpen(new StreamListener(resolve, reject), msgUri);
            }, true, {
              partsOnDemand: false,
        ...
        async createTab(createTabProperties) {
          ...
          getWindowFromId(createTabProperties.windowId).document.getElementById("tabmail").openTab(createTabProperties.type, params);
        },
        ...
        async resetMessagePane() {
          for (const win of Services.wm.getEnumerator("mail:3pane")) {
            ...
            if (messagepane.contentDocument.documentURI.includes("stub.xhtml")) {
          ...
          return new Promise(resolve => {
            ...
            MsgHdrToMimeMessage(msgHdr, null, (msgHdr, mimeMsg) => {
              ...
              if (Prefs.extra_attachments) {
                attachments = [...mimeMsg.allAttachments, ...mimeMsg.allUserAttachments];
                let seenMap = new Set();
                ...
                attachments = mimeMsg.allUserAttachments.filter(a => a.isRealAttachment);
              }

              resolve(attachments.map((a, i) => {
                return {
                  size: a.size,
        ...
        async alwaysShowRemoteContent(email) {
          const uri = Services.io.newURI("chrome://messenger/content/email=" + email); // TB 78: If we have createContentPrincipal we're in the TB 78+ code.

          if ("createContentPrincipal" in Services.scriptSecurityManager) {
            Services.perms.addFromPrincipal(Services.scriptSecurityManager.createContentPrincipal(uri, {}), "image", Services.perms.ALLOW_ACTION);
          } else {
        ...
        async beginReply(id, type) {
          ...
          Services.wm.getMostRecentWindow("mail:3pane").ComposeMessage(compType, Ci.nsIMsgCompFormat.Default, msgHdr.folder, [msgHdr.folder.getUriForMsg(msgHdr)]);
        },

        async beginForward(id, type) {
          let msgHdr = context.extension.messageManager.get(id);
          let compType = type == "forwardAsAttachment" ? Ci.nsIMsgCompType.ForwardAsAttachment : Ci.nsIMsgCompType.ForwardInline;
          Services.wm.getMostRecentWindow("mail:3pane").ComposeMessage(compType, Ci.nsIMsgCompFormat.Default, msgHdr.folder, [msgHdr.folder.getUriForMsg(msgHdr)]);
        },

        async beginEdit(id, type) {
          let msgHdr = context.extension.messageManager.get(id);
          let compType = type == "editAsNew" ? Ci.nsIMsgCompType.Template : Ci.nsIMsgCompType.Draft;
          Services.wm.getMostRecentWindow("mail:3pane").ComposeMessage(compType, Ci.nsIMsgCompFormat.Default, msgHdr.folder, [msgHdr.folder.getUriForMsg(msgHdr)]);
        },

        async ignorePhishing(id) {
          let msgHdr = context.extension.messageManager.get(id);
          msgHdr.setUint32Property("notAPhishMessage", 1); // Force a commit of the underlying msgDatabase.
        ...
        async downloadAllAttachments(id) {
          ...
          let attachments = await new Promise(resolve => {
            MsgHdrToMimeMessage(msgHdr, null, async (aMsgHdr, aMimeMsg) => {
              ...
              resolve(aMimeMsg.allUserAttachments.filter(x => x.isRealAttachment));
            });
          });
          const win = Services.wm.getMostRecentWindow("mail:3pane");
          let msgUri = msgHdrGetUri(msgHdr);
          win.HandleMultipleAttachments(attachments.map(att => getAttachmentInfo(win, msgUri, att)), "save");
        },
        ...
        onSetConversationPreferences: new ExtensionCommon.EventManager({
          ...
          register(fire) {
            const overridePrefs = ["mail.inline_attachments", "mailnews.scroll_to_new_message"];
