...
class WindowObserver {
  ...
  observe(subject, topic, data) {
    ...
    waitForWindow(win).then(() => {
      if (win.document.location != "chrome://messenger/content/messenger.xul" && win.document.location != "chrome://messenger/content/messenger.xhtml") {
        return;
      }

      this._callback(subject.window, this._windowManager.getWrapper(subject.window).id);
    });
  }
    ...
    return {
      convMsgWindow: {
        async isSelectionExpanded(windowId) {
          ...
          const msgIndex = win.gFolderDisplay ? win.gFolderDisplay.selectedIndices[0] : -1;

          if (msgIndex >= 0) {
            try {
              let viewThread = win.gDBView.getThreadContainingIndex(msgIndex);
              let rootIndex = win.gDBView.findIndexOfMsgHdr(viewThread.getChildHdrAt(0), false);

              if (rootIndex >= 0) {
                return win.gDBView.isContainer(rootIndex) && !win.gFolderDisplay.view.isCollapsedThreadAtIndex(rootIndex);
              }
            } catch (ex) {
        ...
        async isSelectionThreaded(windowId) {
          const win = getWindowFromId(windowManager, context, windowId); // If we're not showing threaded, then we only worry about how many
        ...
        async getDisplayedMessages(tabId) {
          ...
          if (tab.__proto__.constructor.name == "TabmailTab") {
            if (tab.active && ["folder", "glodaList", "message"].includes(tab.nativeTab.mode.name)) {
        ...
        async openNewWindow(url) {
          const win = getWindowFromId(); // Counting some extra pixels for window decorations.

          let height = Math.min(win.screen.availHeight - 30, 1024);
          win.open(url, "_blank", "chrome,resizable,width=640,height=" + height);
        },
        ...
        onThreadPaneDoubleClick: new ExtensionCommon.EventManager({
          ...
          register(fire) {
            ...
            const windowObserver = new WindowObserver(windowManager, patchDoubleClick);
          ...
          register(fire) {
            const patchMiddleClick = (win, id) => {
              ...
              win.TreeOnMouseDown = async event => {
                if (event.target.parentNode.id !== "threadTree" || event.button != 1) {
                  win.oldTreeOnMouseDown(event);
                  return;
                } // Middle-click


                win.ChangeSelectionWithoutContentLoad(event, event.target.parentNode, false);
                let msgHdrs = win.gFolderDisplay.selectedMessages;
                msgHdrs = msgHdrs.map(hdr => messageManager.convert(hdr));
                let result = await fire.async(id, msgHdrs).catch(console.error);
            ...
            const windowObserver = new WindowObserver(windowManager, patchMiddleClick);
            ...
            return function () {
              ...
              monkeyPatchAllWindows(windowManager, (win, id) => {
                ...
                folderTree.removeEventListener("mousedown", win.TreeOnMouseDown, true);
                folderTree.removeEventListener("mousedown", win.oldTreeOnMouseDown, true);
                win.TreeOnMouseDown = win.oldTreeOnMouseDown;
                delete win.oldTreeOnMouseDown;
                folderTree.addEventListener("mousedown", win.TreeOnMouseDown, true);
              });
            };
          }
        ...
        onMonkeyPatch: new ExtensionCommon.EventManager({
          ...
          register(fire) {
            const windowObserver = new WindowObserver(windowManager, specialPatches);
        ...
        onSummarizeThread: new ExtensionCommon.EventManager({
          ...
          register(fire) {
            const windowObserver = new WindowObserver(windowManager, summarizeThreadHandler);
            ...
            return function () {
              monkeyPatchAllWindows(windowManager, (win, id) => {
                ...
                win.MessageDisplayWidget.prototype.onSelectedMessagesChanged = win.originalOnSelectedMessagesChanged;
...
function getWindowFromId(windowManager, context, id) {
  return id !== null && id !== undefined ? windowManager.get(id, context).window : Services.wm.getMostRecentWindow("mail:3pane");
}
...
const specialPatches = win => {
  ...
  win.document.getElementById("multimessage").setAttribute("context", "mailContext"); // Because we're not even fetching the conversation when the message pane is
  ...
  win.conversationUndoFuncs.push(() => win.removeEventListener("messagepane-unhide", unhideListener, true));
  let oldSummarizeMultipleSelection = win.summarizeMultipleSelection;

  win.summarizeMultipleSelection = function _summarizeMultiple_patched(aSelectedMessages, aListener) {
  ...
  win.conversationUndoFuncs.push(() => win.summarizeMultipleSelection = oldSummarizeMultipleSelection); // Ok, this is slightly tricky. The C++ code notifies the global msgWindow
  ...
  win.messageHeaderSink.onMsgHasRemoteContent = function (msgHdr, contentURI, canOverride) {
    ...
    if (msgListeners.has(messageId)) {
      ...
      msgListeners.set(messageId, listeners.filter(x => x.get() != null));
  ...
  win.conversationUndoFuncs.push(() => win.messageHeaderSink.onMsgHasRemoteContent = win.oldOnMsgHasRemoteContent);
  ...
  win.conversationUndoFuncs.push(() => messagepane.removeEventListener("load", fightAboutBlank, true));
  fightAboutBlank(); // Never allow prefetch, as we don't want to leak for pages.
...
function summarizeThreadHandler(win, id) {
  ...
  win.oldSummarizeThread = win.summarizeThread; // This one completely nukes the original summarizeThread function, which is
  //  actually the entry point to the original ThreadSummary class.

  win.summarizeThread = function _summarizeThread_patched(aSelectedMessages, aListener) {
    ...
    win.gSummaryFrameManager.loadAndCallback("chrome://conversations/content/stub.xhtml", function (isRefresh) {
      ...
      if (isRefresh) {
        ...
        previouslySelectedUris = []; // Invalidate any remaining conversation
        ...
        htmlpane.contentWindow.Conversations = win.Conversations; // The DOM window is fresh, it needs an event listener to forward
        ...
        htmlpane.contentWindow.addEventListener("keypress", function (event) {
          ...
          } catch (e) {// Log.debug("We failed to dispatch the event, don't know why...", e);
          }
        });
      }
      ...
      (async () => {
        ...
        let isSelectionThreaded = await browser.convMsgWindow.isSelectionThreaded(this._windowId);
        ...
        if (isSubSetOrEqual(previouslySelectedUris, newlySelectedUris) && previousIsSelectionThreaded == isSelectionThreaded) {
        ...
        let freshConversation = new Conversation(win, aSelectedMessages, isSelectionThreaded, ++win.Conversations.counter);
        Log.debug("New conversation:", freshConversation.counter, "Old conversation:", win.Conversations.currentConversation && win.Conversations.currentConversation.counter);
        ...
        freshConversation.outputInto(htmlpane.contentWindow, async function (aConversation) {
          ...
          Log.assert(aConversation.isSelectionThreaded == isSelectionThreaded, "Someone forgot to put the right scroll mode!"); // Here, put the final touches to our new conversation object.
          ...
          if (Services.prefs.getBoolPref("mailnews.mark_message_read.auto")) {
            win.conversationsMarkReadTimeout = win.setTimeout(async function () {
              ...
              if (isSelectionThreaded) {
                ...
                for (const msgHdr of aSelectedMessages) {
                  const id = await browser.conversations.getMessageIdForUri(msgHdrGetUri(msgHdr));
            ...
            }, Services.prefs.getIntPref("mailnews.mark_message_read.delay.interval") * Services.prefs.getBoolPref("mailnews.mark_message_read.delay") * 1000);
          }
        });
      })().catch(console.error);
    });
  ...
  win.originalOnSelectedMessagesChanged = win.MessageDisplayWidget.prototype.onSelectedMessagesChanged;
  ...
  win.MessageDisplayWidget.prototype.onSelectedMessagesChanged = function _onSelectedMessagesChanged_patched() {
    try {
      ...
      if (selectedCount == 0) {
        ...
        let msgHdr = this.folderDisplay.selectedMessage; // We can't display NTTP messages and RSS messages properly yet, so
