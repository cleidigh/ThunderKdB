...
async function glodaQuery(options = {
  kind: "email",
  value: null,
  query: "NOUN_IDENTITY",
  involvesItems: null
    ...
    return {
      convContacts: {
        async beginNew(beginNewProperties) {
          const window = getWindowFromId(windowManager, context, beginNewProperties.windowId);
          const args = {};
          ...
          let dialog = "chrome://messenger/content/addressbook/abNewCardDialog.xhtml";

          if (Services.vc.compare(Services.appinfo.version, "77.0.0") < 0) {
            dialog = "chrome://messenger/content/addressbook/abNewCardDialog.xul";
          }

          window.openDialog(dialog, "", "chrome,resizable=no,titlebar,modal,centerscreen", args);
        },

        async beginEdit(beginEditProperties) {
          const window = getWindowFromId(windowManager, context, beginEditProperties.windowId);
          let cardAndBook = DisplayNameUtils.getCardForEmail(beginEditProperties.email);
          const args = {
            abURI: cardAndBook.book.URI,
            card: cardAndBook.card
          };
          let dialog = "chrome://messenger/content/addressbook/abEditCardDialog.xhtml";

          if (Services.vc.compare(Services.appinfo.version, "77.0.0") < 0) {
            dialog = "chrome://messenger/content/addressbook/abEditCardDialog.xul";
          }

          window.openDialog(dialog, "", "chrome,modal,resizable=no,centerscreen", args);
        },

        async composeNew(properties) {
          const window = getWindowFromId(windowManager, context, properties.windowId);
          const {
            to
          } = properties;
        ...
        async showMessagesInvolving(options) {
          const window = getWindowFromId(windowManager, context, options.windowId);
        ...
        async makeMimeAddress(options) {
          ...
          return !name || name == email ? email : MailServices.headerParser.makeMimeAddress(name, email);
        },

        async getIdentityEmails(options) {
          const {
            includeNntpIdentities
          } = options;
        ...
        onColumnHandler: new ExtensionCommon.EventManager({
          ...
          register(fire, columnName, columnTooltip, betweenMeAndSomeone, betweenSomeoneAndMe, commaSeparator, andSeparator) {
            let callback = createColumn.bind(null, columnName, columnTooltip);
            const windowObserver = new WindowObserverContacts(windowManager, callback);
            monkeyPatchAllWindows(windowManager, callback);
            Services.ww.registerNotification(windowObserver);
            const emails = getIdentityEmailsImpl();
            let callback2 = registerColumn.bind(null, emails, betweenMeAndSomeone, betweenSomeoneAndMe, commaSeparator, andSeparator);
            const windowObserver2 = new WindowObserverContacts(windowManager, callback2);
...
function createColumn(columnName, columnTooltip, win, id) {
  ...
  [["id", "betweenCol"], ["flex", "4"], ["persist", "width hidden ordinal"], ["label", columnName], ["tooltiptext", columnTooltip]].forEach(function ([k, v]) {
  ...
  win.setTimeout(() => {
    if (!Services.prefs.getBoolPref("conversations.betweenColumnVisible", true)) {
...
async function registerColumn(emails, betweenMeAndSomeone, betweenSomeoneAndMe, commaSeparator, andSeparator, win, id) {
  ...
  let participants = function (msgHdr) {
    try {
      ...
      let people = new Set(); // Helper for formatting; depending on the locale, we may need a different
      ...
      addPeople("bccList", false); // And turn this into a human-readable line.
  ...
  try {
    win.gDBView.addColumnHandler("betweenCol", columnHandler);
  } catch (e) {// This is really weird, but rkent does it for junquilla, and this solves
  ...
  win.addEventListener("beforeunload", () => {
    ...
    if (col) {
      ...
      Services.prefs.setBoolPref("conversations.betweenColumnVisible", isHidden != "true");
    }
  }, {
    once: true
  });
...
function composeMessageTo(aEmail, aDisplayedFolder) {
  ...
  if (aDisplayedFolder) {
    params.identity = MailServices.accounts.getFirstIdentityForServer(aDisplayedFolder.server);
  }
    ...
    if (aTopic == "domwindowopened") {
      if (aSubject && "QueryInterface" in aSubject) {
        ...
        waitForWindow(win).then(() => {
          ...
          this._callback(aSubject.window, this._windowManager.getWrapper(aSubject.window).id);
        });
      }
    }
  }
