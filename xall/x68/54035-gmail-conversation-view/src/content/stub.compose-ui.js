...
var {
  ...
  msgHdrIsArchive,
  msgHdrGetUri
} = ChromeUtils.import("chrome://conversations/content/modules/stdlib/msgHdrUtils.js");
var {
  sendMessage
} = ChromeUtils.import("chrome://conversations/content/modules/stdlib/send.js");
var {
  composeInIframe,
  htmlToPlainText,
  replyAllParams,
  parse
} = ChromeUtils.import("chrome://conversations/content/modules/stdlib/compose.js");
var {
  getHooks
...
var {
  SimpleStorage
} = ChromeUtils.import("chrome://conversations/content/modules/stdlib/SimpleStorage.js", {});
...
function registerQuickReply() {
  ...
  if (!(id in mainWindow.Conversations.draftListeners)) {
    mainWindow.Conversations.createDraftListenerArrayForId(id); // We can't use the (seemingly) simple line below because the array would be
  ...
  registerQuickReplyEventListeners(); // Bold, italics, etC.
...
function newComposeSessionByClick(type) {
  Log.assert(!gComposeSession, "We should only get here if there's no compose session already");
  Log.debug("Setting up the initial quick reply compose parameters...");

  try {
    gComposeSession = createComposeSession(x => x.reply(getMessageForQuickReply(), type)); // This could probably be refined, like only considering we started editing
...
function ComposeSession(match) {
  ...
  this.match = match; // A composition session may be setup (i.e. the fields in the UI filled with
  //  the right values), but that doesn't mean the user has edited it yet...

  this.startedEditing = false; // Shall we archive this conversation after sending?

  this.archive = false; // These are the parameters that are defined once at the beginning of the
...
ComposeSession.prototype = {
  ...
  setupIdentity() {
    ...
    this.match({
      ...
      new() {
        ...
        if (selectedFolder) {
          identity = selectedFolder.customIdentity || topMail3Pane(window).getIdentityForServer(selectedFolder.server) || identity;
        } // We're done!
  ...
  setupFinal() {
    ...
    this.match({
      reply(aMessage, aReplyType) {
        let aMsgHdr = aMessage._msgHdr; // Can't use getActiveEditor() at this stage because gComposeSession
        // hasn't been set yet.

        let iframe = aReplyType == "reply" ? document.querySelector("li.reply .textarea") : document.querySelector("li.replyAll .textarea");
  ...
  send(options) {
    ...
    sendMessage({
      ...
      progressListener,
      sendListener,
      stateListener: createStateListener(self, Conversations.currentConversation.msgHdrs, Conversations.currentConversation.id)
    }, {
      popOut,
...
nsAttachmentOpener.prototype = {
  QueryInterface: ChromeUtils.generateQI([Ci.nsIURIContentListener, Ci.nsIInterfaceRequestor]),
...
AttachmentList.prototype = {
  ...
  addWithData(aData) {
    let msgAttachment = Cc["@mozilla.org/messengercompose/attachment;1"].createInstance(Ci.nsIMsgAttachment);
...
function attachmentDataFromDragData(event) {
  ...
  let messageData = event.dataTransfer.getData("text/x-moz-message"); // Log.debug("file", fileData, "url", urlData, "message", messageData);
  ...
  if (fileData || urlData || messageData) {
    ...
    if (messageData) {
      size = topMail3Pane(window).messenger.messageServiceFromURI(messageData).messageURIToMsgHdr(messageData).messageSize;
      url = messageData;
    ...
    if (urlData) {
      try {
        let scheme = Services.io.extractScheme(url); // don't attach mailto: urls
...
let progressListener = {
  ...
  onProgressChange(aWebProgress, aRequest, aCurSelfProgress, aMaxSelfProgress, aCurTotalProgress, aMaxTotalProgress) {
    Log.debug("onProgressChange", aWebProgress, aRequest, aCurSelfProgress, aMaxSelfProgress, aCurTotalProgress, aMaxTotalProgress); // Calculate percentage.
  ...
  onLocationChange(aWebProgress, aRequest, aLocation) {// we can ignore this notification
  },
  ...
  onSecurityChange(aWebProgress, aRequest, state) {// we can ignore this notification
  },
...
let sendListener = {
  ...
  onStopSending(aMsgID, aStatus, aMsg, aReturnFile) {
    ...
    $("textarea, #send, #sendArchive").removeAttr("disabled"); // This function is called only when the actual send has been performed,
...
function createStateListener(aComposeSession, aMsgHdrs, aId) {
  return {
    NotifyComposeFieldsReady() {// ComposeFieldsReady();
    },

    NotifyComposeBodyReady() {// if (gMsgCompose.composeHTML)
      //   loadHTMLMsgPrefs();
      // AdjustFocus();
    },
    ...
    ComposeProcessDone(aResult) {
      ...
      if (NS_SUCCEEDED(aResult)) {
        ...
        if (!isQuickCompose && aComposeSession == gComposeSession) {
          ...
          getActiveEditor().value = "";
          $(".quickReplyHeader").hide();
          hideQuickReply(); // We can do this because we're in the right if-block.
    ...
    SaveInFolderDone(folderURI) {// DisplaySaveFolderDlg(folderURI);
    }
...
function masqueradeAsQuickCompose() {
  ...
  $(".replyAll, #save, .replyMethod").remove(); // TODO figure out why this timeout is needed
  ...
  let data = []; // Push a new contact item in the list
  ...
  let pushNewPopularContacts = function (n) {
    ...
    items.forEach(function (data2, i) {
      ...
      node.find(".popularRemove").click(function () {
        Log.debug("Removing", data.name, data.email); // Mark it in the prefs
        ...
        Prefs.setString("conversations.unwanted_recipients", JSON.stringify(unwantedRecipients)); // Update the UI
      ...
      node.find(".popularName").click(function () {
        ...
        let bcc = JSON.parse($("#bcc").val()); // Append our new value

        to.push(MailServices.headerParser.makeMimeAddress(data.name, data.email)); // Re-set everything

        let format = items => items.map(parseMimeLine).map(([{
          name,
          email
        }]) => asToken(null, name, email, null));

        setupAutocomplete(format(to), format(cc), format(bcc)); // Remove the node!
  ...
  $(".quickReplyContactsMoreLink").click(() => pushNewPopularContacts(10)); // Fill in the "10 most popular contacts" thing
  ...
  contactCollection.becomeExplicit(); // Misc
