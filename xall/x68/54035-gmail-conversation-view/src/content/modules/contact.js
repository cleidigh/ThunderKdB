...
ContactManager.prototype = {
  async getContactFromNameAndEmail(name, email) {
    ...
    email = (email + "").toLowerCase(); // Might change in the future... who knows? ...
    ...
    await contact.fetch(); // Only cache contacts which are in the address book. This avoids weird
...
ContactFromAB.prototype = {
  async fetch() {
    ...
    let card = matchingCards.length !== 0 ? { ...matchingCards[0].properties,
    ...
    if (card) {
      ...
      this._useCardName = card.PreferDisplayName != null ? !!+card.PreferDisplayName : true;
      this.emails = [card.PrimaryEmail, card.SecondEmail || ""]; // Prefer:
      // - displayName
  ...
  async toTmplData(position, email, isDetail) {
    const identityEmails = await browser.convContacts.getIdentityEmails({
      includeNntpIdentities: false
    }).catch(console.error);

    const lcEmail = this._email.toLowerCase();

    const hasIdentity = identityEmails.find(e => e.toLowerCase() == lcEmail); // `name` and `extra` are the only attributes that depend on `position`

    let name = this._name || this._email;
    let extra = "";

    if (!isDetail && hasIdentity) {
      name = position === Contacts.kFrom ? browser.i18n.getMessage("message.meFromMeToSomeone") : browser.i18n.getMessage("message.meFromSomeoneToMe");
      extra = this._email;
    }

    const displayEmail = name != email ? email : "";
    const skipEmail = !isDetail && this._card && (await browser.conversations.getCorePref("mail.showCondensedAddresses"));
