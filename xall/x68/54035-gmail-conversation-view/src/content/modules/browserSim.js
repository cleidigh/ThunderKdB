...
var EXPORTED_SYMBOLS = ["BrowserSim"]; // This is a workaround whilst we still have stub.xhtml being loaded in the
...
const SUPPORTED_BASE_APIS = ["convContacts", "convMsgWindow", "conversations", "i18n", "messageDisplay", "messages", "runtime", "storage", "tabs", "compose", "contacts", "addressBooks"];
...
class _BrowserSim {
  ...
  getBrowser() {
    ...
    for (const apiName of SUPPORTED_BASE_APIS) {
      if (apiName == "i18n") {
        let api = extension.apiManager.getAPI(apiName, extension, "addon_parent");
  ...
  async getBrowserAsync() {
    ...
    for (const apiName of SUPPORTED_BASE_APIS) {
      if (apiName == "i18n") {
        ...
        const asyncAPI = await extension.apiManager.asyncGetAPI( // contacts and addressBooks are actually contained within the same
        // API module.
        apiName == "contacts" || apiName == "addressBooks" ? "addressBook" : apiName, extension, "addon_parent");
