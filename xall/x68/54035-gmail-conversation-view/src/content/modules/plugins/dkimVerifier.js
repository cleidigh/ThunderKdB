...
try {
  AuthVerifier = ChromeUtils.import("resource://dkim_verifier/AuthVerifier.jsm").AuthVerifier;
...
if (hasDKIMVerifier) {
  ...
  mailWindow.DKIM_Verifier.Display.onEndHeaders = function () {
    "use strict"; // don't start a verification for the classic view if it is not shown
...
let dkimVerifierHook = {
  onMessageStreamed(msgHdr, unused, mainWindow, msg) {
    ...
    AuthVerifier.verify(msgHdr).then(result => {
