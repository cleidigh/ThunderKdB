...
let Log = setupLogging("Conversations.Modules.Enigmail"); // Enigmail support, thanks to Patrick Brunschwig!
...
let enigmailSvc; // used in enigmailMsgComposeOverlay.js
...
let global = this; // eslint-disable-next-line no-redeclare
...
if (hasEnigmail) {
  ...
  try {
    ...
    loader.loadSubScript(`chrome://openpgp/content/ui/enigmailMsgComposeOverlay.js`, global);
    loader.loadSubScript(`chrome://openpgp/content/ui/enigmailMsgComposeHelper.js`, global);
  } catch (e) {
    hasEnigmail = false;
  ...
  messagepane.addEventListener("load", function _overrideUpdateSecurity() {
    ...
    if (w.Enigmail.hdrView) {
      ...
      w.addEventListener("load-enigmail", () => {
        overrideUpdateSecurity(messagepane, w);
      }, {
        once: true,
        capture: true
...
function overrideUpdateSecurity(messagepane, w) {
  ...
  let originalUpdateSecurityStatus = headerSink.updateSecurityStatus; // Called after decryption or verification is completed.
  // Security status of a message is updated and shown at the status bar
  // and the header box.

  headerSink.updateSecurityStatus = function (unusedUriSpec, exitCode, statusFlags, extStatusFlags, keyId, userId, sigDetails, errorMsg, blockSeparation, uri, extraDetails, mimePartNumber) {
    ...
    let updateHdrIcons = function () {
      w.Enigmail.hdrView.updateHdrIcons(exitCode, statusFlags, extStatusFlags, keyId, userId, sigDetails, errorMsg, blockSeparation, encToDetails, null); // xtraStatus
    };

    showHdrIconsOnStreamed(message, updateHdrIcons); // Show signed label of encrypted and signed pgp/mime.
...
function tryEnigmail(aDocument, aMessage, aMsgWindow) {
  ...
  try {
    ...
    let treeWalker = aDocument.createTreeWalker(bodyElement, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
    ...
    Log.debug("charset=" + charset); // Encode ciphertext to charset from unicode
    ...
    let retry = charset != "UTF-8" ? 1 : 2; // extract text preceeding and/or following armored block
    ...
    var uiFlags = 0;
    var plainText = EnigmailDecryption.decryptMessage(window, uiFlags, msgText, signatureObj, exitCodeObj, statusFlagsObj, keyIdObj, userIdObj, sigDetailsObj, errorMsgObj, blockSeparationObj, encToDetailsObj);
    var exitCode = exitCodeObj.value; // do not return anything if gpg signales DECRYPTION_FAILED
    // (which could be possible in case of MDC errors)

    if (uiFlags & EnigmailConstants.UI_IGNORE_MDC_ERROR && statusFlagsObj.value & EnigmailConstants.MISSING_MDC) {
    ...
    if (exitCode == 0) {
      if (msgRfc822Text.length) {
        let node = bodyElement.querySelector("div.moz-text-plain"); // If there's no suitable node to put the decrypted text in, create one
        ...
        msgRfc822Text = EnigmailData.convertToUnicode(msgRfc822Text, charset); // eslint-disable-next-line no-unsanitized/property
    ...
    showHdrIconsOnStreamed(aMessage, function () {
      w.Enigmail.hdrView.updateHdrIcons(exitCode, statusFlagsObj.value, 0, keyIdObj.value, userIdObj.value, sigDetailsObj.value, errorMsgObj.value, blockSeparationObj.value, encToDetailsObj.value);
    });
    return statusFlagsObj.value;
  } catch (ex) {
...
function verifyAttachments(aMessage) {
  ...
  w.Enigmail.msg.verifyEmbeddedMsg(w, mailNewsUrl, w.msgWindow, uri, null, null);
...
function showHdrIconsOnStreamed(message, updateHdrIcons) {
  ...
  w.Enigmail.hdrView.statusBarHide();
  updateHdrIcons(); // Prepare for showing on focus.
...
function patchForShowSecurityInfo(aWindow) {
  ...
  let oldTreeController = w.top.controllers.getControllerForCommand("button_enigmail_decrypt");
  ...
  w._newTreeController = treeController; // Event listeners are added in enigmailMsgHdrViewOverlay.js,
...
function addSignedLabel(status, msg) {
  if (status & (EnigmailConstants.BAD_SIGNATURE | EnigmailConstants.GOOD_SIGNATURE | EnigmailConstants.EXPIRED_KEY_SIGNATURE | EnigmailConstants.EXPIRED_SIGNATURE | EnigmailConstants.UNVERIFIED_SIGNATURE | EnigmailConstants.REVOKED_KEY | EnigmailConstants.EXPIRED_KEY_SIGNATURE | EnigmailConstants.EXPIRED_SIGNATURE)) {
    msg.addSpecialTag({
      canClick: true,
      ...
      title: status & EnigmailConstants.UNVERIFIED_SIGNATURE ? browser.i18n.getMessage("enigmail.unknownGood") : browser.i18n.getMessage("enigmail.messageSignedLong")
    });
  }
}
...
let enigmailHook = {
  _domNode: null,
  _originalText: null,
  // for restoring original text when sending message is canceled
  _currentlyStreaming: [],
  ...
  onMessageStreamed(msgHdr, iframe, mainWindow, message) {
    ...
    this._currentlyStreaming = this._currentlyStreaming.filter(uri => uri == message._uri);
  ...
  // eslint-disable-next-line complexity
  onMessageBeforeSendOrPopout(aAddress, aEditor, aStatus, aPopout, aAttachmentList, aWindow) {
    if (hasEnigmail) {
    ...
    Enigmail.msg.processAccountSpecificDefaultOptions(); // Get flags from UI checkboxes.
    ...
    if (EnigmailPrefs.getPref("encryptToSelf") || sendFlags & EnigmailConstants.SAVE_MESSAGE) {
      optSendFlags |= EnigmailConstants.SEND_ENCRYPT_TO_SELF;
    }

    sendFlags |= optSendFlags;
    let toAddrList = aAddress.to.concat(aAddress.cc).map(EnigmailFuncs.stripEmail);
    let bccAddrList = aAddress.bcc.map(EnigmailFuncs.stripEmail);
    let result = Enigmail.msg.keySelection(enigmailSvc, sendFlags, // all current combined/processed send flags (incl. optSendFlags)
    ...
    try {
      let origText;
      let usingPGPMime = sendFlags & EnigmailConstants.SEND_PGP_MIME && sendFlags & (ENCRYPT | SIGN);

      if (!usingPGPMime && sendFlags & ENCRYPT && aAttachmentList && aAttachmentList.attachments.length) {
        // Attachments will not be encrypted using inline-PGP.
        // We switch to PGP/MIME if possible.
        if (EnigmailDialog.confirmDlg(window, browser.i18n.getMessage("enigmail.attachmentsNotEncrypted"), EnigmailLocale.getString("pgpMime_sMime.dlg.pgpMime.button"), EnigmailLocale.getString("dlg.button.cancel"))) {
          usingPGPMime = true;
          sendFlags |= EnigmailConstants.SEND_PGP_MIME;
        } else {
          aStatus.canceled = true;
      ...
      if (usingPGPMime) {
        ...
        let cipherText = EnigmailEncryption.encryptMessage(window, uiFlags, plainText, fromAddr, toAddr, bccAddr, sendFlags, exitCodeObj, statusFlagsObj, errorMsgObj);
        let exitCode = exitCodeObj.value;

        if (cipherText && exitCode == 0) {
          if (sendFlags & ENCRYPT && charset && charset.search(/^us-ascii$/i) != 0) {
          ...
          aStatus.canceled = !EnigmailDialog.confirmDlg(window, msg, EnigmailLocale.getString("msgCompose.button.sendUnencrypted"));
          return aStatus;
        }
      }

      if (!(sendFlags & EnigmailConstants.SAVE_MESSAGE) && EnigmailPrefs.getPref("confirmBeforeSending")) {
        if (!Enigmail.msg.confirmBeforeSend(toAddrList.join(", "), toAddr + ", " + bccAddr, sendFlags, false)) {
          if (origText) {
  ...
  onMessageBeforeSendOrPopout_canceled(aAddress, aEditor, aStatus, aPopout, aAttachmentList) {
  ...
  onComposeSessionChanged(aComposeSession, aMessage, aAddress, aEditor, aWindow) {
    ...
    aWindow.document.querySelector(".enigmail").style.display = "inline-block"; // Get default decrypt, sign and PGP/MIME status from settings.
    ...
    Enigmail.msg.pgpmimeForced = EnigmailConstants.ENIG_UNDEF; // Set Enigmail.msg.sendMode from identity

    Enigmail.msg.identity = aComposeSession.params.identity;

    Enigmail.msg.setOwnKeyStatus = function () {};

    Enigmail.msg.processAccountSpecificDefaultOptions(); // Set sendMode from messages
    ...
    if (toAddrList.length && EnigmailPrefs.getPref("assignKeysByRules")) {
      let matchedKeysObj = {};
      let flagsObj = {};
      let success = EnigmailRules.mapAddrsToKeys(toAddrList.join(", "), false, // no interaction if not all addrs have a key
      window, matchedKeysObj, // resulting matching keys
    ...
    replySign.setAttribute("title", Enigmail.msg.reasonSigned); // Add listeners to set final mode
    ...
    waitLoadingBody(function () {
      ...
      aEditor.node.contentDocument.querySelector("blockquote").innerHTML = escapeHtml(aMessage.decryptedText).replace(/\r?\n/g, "<br>");
    });
  },
