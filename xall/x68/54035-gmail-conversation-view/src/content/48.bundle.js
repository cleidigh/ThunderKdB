"use strict";
(self["webpackChunkthunderbirdconversations"] = self["webpackChunkthunderbirdconversations"] || []).push([[48],{
...
var thunderbird_compat = __webpack_require__(2707);
...
async function enrichWithDisplayData({
  ...
  if (contact.identityId !== undefined) {
    name = field === "from" ? thunderbird_compat/* browser.i18n.getMessage */.Xh.i18n.getMessage("message.meFromMeToSomeone") : thunderbird_compat/* browser.i18n.getMessage */.Xh.i18n.getMessage("message.meFromSomeoneToMe");
  }
...
async function mergeContactDetails(msgData) {
  let showCondensed = await thunderbird_compat/* browser.conversations.getCorePref */.Xh.conversations.getCorePref("mail.showCondensedAddresses"); // Build a map of all the contacts in the thread and de-dupe to avoid
  ...
  for (const message of msgData) {
    for (const contacts of Object.values(message._contactsData)) {
      for (const contact of contacts) {
        ...
        contactMap.set(contact.email, // This is designed to not await on the request. However, in the
        ...
        await thunderbird_compat/* browser._background.request */.Xh._background.request({
          type: "contactDetails",
          payload: contact
        }));
      }
  ...
  for (const message of msgData) {
    ...
    for (const [field, contacts] of Object.entries(message._contactsData)) {
      const contactData = await Promise.all(contacts.map(async contact => [await contactMap.get(contact.email), // We need to keep the raw email around to format the data correctly
...
let messageEnricher = new class {
  constructor() {
    ...
    this.pluralForm = thunderbird_compat/* browser.i18n.getMessage */.Xh.i18n.getMessage("pluralForm");
    this.numAttachmentsString = thunderbird_compat/* browser.i18n.getMessage */.Xh.i18n.getMessage("attachments.numAttachments");
    this.sizeUnknownString = thunderbird_compat/* browser.i18n.getMessage */.Xh.i18n.getMessage("attachments.sizeUnknown");
  }
  /**
  ...
  async enrich(mode, msgData, summary, selectedMessages) {
    const userTags = await thunderbird_compat/* browser.messages.listTags */.Xh.messages.listTags();
    await Promise.all(msgData.map(async message => {
      try {
        await this._addDetailsFromHeader(summary.tabId, message, userTags, selectedMessages);
        await this._addDetailsFromAttachments(message, summary.prefs.extraAttachments);
    ...
    if (mode != "replaceMsg") {
      if (mode == "replaceAll") {
        ...
        this._expandAndScroll(msgData, selectedMessages, summary.tabId, summary.prefs.expandWho);
      } else {
        this._markMsgsToExpand(msgData, selectedMessages, -1, summary.prefs.expandWho, mode);
      }
    }
  }
  /**
  ...
  _filterOutDuplicatesAndInvalids(msgData) {
    ...
    for (let group of groupedMessages.values()) {
      ...
      let msg = // If it doesn't have a folderName it is in view.
      findForCriterion(msg => !msg.folderName) || findForCriterion(msg => msg.isInbox) || findForCriterion(msg => msg.isSent) || findForCriterion(msg => msg.isArchives) || // Worst case, fallback to the first one.
      group[0]; // The message that's selected has the highest priority to avoid
  ...
  _expandAndScroll(msgData, selectedMessages, tabId, expandWho) {
    ...
    this._markMsgsToExpand(msgData, selectedMessages, focusThis, expandWho, "expandAll");
  }
  /**
  ...
  _whereToScroll(msgData, selectedMessages) {
    let needsScroll = -1; // Conversations stub UI is only displayed when a thread is selected,
  ...
  _markMsgsToExpand(msgData, selectedMessages, focusIndex, expandWho, mode) {
    switch (expandWho) {
      default:
        console.error(false, `Unknown value '${expandWho}' for pref expandWho, try changing in the add-on preferences.`);
  ...
  async _addDetailsFromHeader(tabId, message, userTags, selectedMessages) {
    const messageHeader = await thunderbird_compat/* browser.messages.get */.Xh.messages.get(message.id);
    ...
    let isInView = selectedMessages.some(id => id == message.id) || (await thunderbird_compat/* browser.conversations.isInView */.Xh.conversations.isInView(tabId, message.id));

    if (!isInView) {
      message.folderName = await thunderbird_compat/* browser.conversations.getFolderName */.Xh.conversations.getFolderName(message.id);
  ...
  async _addDetailsFromAttachments(message, extraAttachments) {
    ...
    for (let i = 0; i < l; i++) {
      const att = attachments[i]; // This is bug 630011, remove when fixed

      let formattedSize = this.sizeUnknownString; // -1 means size unknown

      if (att.size != -1) {
        formattedSize = await thunderbird_compat/* browser.conversations.formatFileSize */.Xh.conversations.formatFileSize(att.size);
    ...
    message.attachmentsPlural = l ? await thunderbird_compat/* browser.conversations.makePlural */.Xh.conversations.makePlural(this.pluralForm, this.numAttachmentsString, l) : "";
  }
  /**
  ...
  dateAsInMessageList(date) {
    const now = new Date(); // Is it today?
  ...
  async _setDates(message, summary) {
    ...
    if (summary.prefs.noFriendlyDate) {
      ...
      message.date = await thunderbird_compat/* browser.conversations.makeFriendlyDateAgo */.Xh.conversations.makeFriendlyDateAgo(date.getTime());
...
;// CONCATENATED MODULE: ./addon/content/reducer/controllerActions.js
/* This Source Code Form is subject to the terms of the Mozilla Public
 * License, v. 2.0. If a copy of the MPL was not distributed with this
 * file, You can obtain one at https://mozilla.org/MPL/2.0/. */
...
async function handleShowDetails(messages, state, dispatch, updateFn) {
  ...
  if (defaultShowing) {
    for (let msg of state.messages.msgData) {
      await dispatch(reducer_messages/* messageActions.showMsgDetails */.od.showMsgDetails({
        id: msg.id,
        detailsShowing: true
      }));
    }
...
async function setupConversationInTab(params, isInTab) {
  ...
  if (!msgIds.length) {
    ...
    msgUrls, ++window.Conversations.counter, isInTab);
    ...
    freshConversation.outputInto(window, async function (aConversation) {
      ...
      window.Conversations.currentConversation = aConversation; // Create a new rule that will override the default rule, so that
...
const controllerActions = {
  waitForStartup() {
    return async (dispatch, getState) => {
      ...
      const topWin = topMail3Pane(window); // Note: Moving this to after the check for started below is dangerous,
      // since it introduces races where `Conversation` doesn't wait for the
      // page to startup, and hence tab id isn't set.

      await dispatch(reducer_summary/* summaryActions.setConversationState */.vX.setConversationState({
        isInTab,
        isStandalone,
        tabId: BrowserSim.getTabId(topWin, window),
        windowId: BrowserSim.getWindowId(topWin)
      }));
      await dispatch(reducer_summary/* summaryActions.setupUserPreferences */.vX.setupUserPreferences());
      ...
      await dispatch(reducer_summary/* summaryActions.setSystemOptions */.vX.setSystemOptions({
      ...
      await dispatch(reducer_summary/* summaryActions.setupListeners */.vX.setupListeners());
  ...
  updateConversation({
    ...
    return async (dispatch, getState) => {
      ...
      await handleShowDetails(messages, state, dispatch, async () => {
        ...
        await messageEnricher.enrich(mode, messages.msgData, state.summary, mode == "replaceAll" ? summary.initialSet : state.summary.initialSet); // The messages inside `msgData` don't come with filled in `to`/`from`/ect. fields.
        // We need to fill them in ourselves.

        await mergeContactDetails(messages.msgData);

        if (mode == "replaceAll") {
          summary.subject = messages.msgData[messages.msgData.length - 1]?.subject;
          await dispatch(reducer_compose/* composeSlice.actions.resetStore */.jz.actions.resetStore());
          await dispatch(reducer_quickReply/* quickReplySlice.actions.setExpandedState */.PK.actions.setExpandedState({
            expanded: false
          }));
          await dispatch(reducer_summary/* summaryActions.replaceSummaryDetails */.vX.replaceSummaryDetails(summary));
        }

        await dispatch(reducer_messages/* messageActions.updateConversation */.od.updateConversation({
          messages,
          mode
        }));
        ...
        if (mode == "replaceAll") {
          if (state.summary.prefs.loggingEnabled) {
            console.debug("Load took (ms):", Date.now() - summary.loadingStartedTime);
          }

          await dispatch(reducer_summary/* summaryActions.setMarkAsRead */.vX.setMarkAsRead());
        }
      });
    };
  }
...
var reducer_attachments = __webpack_require__(5107);
...
AttachmentMoreMenu.propTypes = {
  detachCallback: (prop_types_default()).func.isRequired,
  deleteCallback: (prop_types_default()).func.isRequired
};
/**
...
class Attachment extends react.PureComponent {
  ...
  render() {
    ...
    return /*#__PURE__*/react.createElement("li", {
      ...
      hash: "more_vert"
    })), this.state.displayMenu && /*#__PURE__*/react.createElement(AttachmentMoreMenu, {
...
function _ContactDetail({
  ...
  if (contactId) {
    contactEdit = contactIsReadOnly ? /*#__PURE__*/react.createElement("button", {
      className: "viewContact",
      title: thunderbird_compat/* browser.i18n.getMessage */.Xh.i18n.getMessage("contact.viewContactTooltip"),
      onClick: editContact
    }, /*#__PURE__*/react.createElement(SvgIcon, {
      hash: "person"
    })) : /*#__PURE__*/react.createElement("button", {
...
_ContactDetail.propTypes = {
  ...
  contactIsReadOnly: (prop_types_default()).bool,
  dispatch: (prop_types_default()).func.isRequired,
  email: (prop_types_default()).string.isRequired,
  msgId: (prop_types_default()).number.isRequired,
  name: (prop_types_default()).string.isRequired,
...
SpecialMessageTags.propTypes = {
  ...
  folderName: (prop_types_default()).string,
...
MessageHeader.propTypes = {
  ...
  recipientsIncludeLists: (prop_types_default()).bool.isRequired,
  isDraft: (prop_types_default()).bool.isRequired,
  shortFolderName: (prop_types_default()).string,
...
class MessageIFrame extends react.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (startLoad && thunderbird_compat/* isWebextension */.sE) {
      ...
      if ("hintCharacterSet" in cv) {
        ...
        docShell.charset = "UTF-8"; // This used to be kCharsetFromChannel = 11, however in 79/80 the code changed.
  ...
  componentDidMount() {
    ...
    if ("hintCharacterSet" in cv) {
      ...
      cv.hintCharacterSetSource = kCharsetFromUserForced; // Thunderbird 78.
      ...
      if ((!this.props.isInTab || this.props.isStandalone) && window.browsingContext) {
        window.browsingContext.embedderElement.addEventListener("click", this.onClickIframe);
      }
  ...
  componentWillUnmount() {
    ...
    if (this._isTB91) {
      // Thunderbird 91
      window.browsingContext.embedderElement.removeEventListener("click", this.onClickIframe);
      window.browsingContext.embedderElement.removeEventListener("DOMContentLoaded", this._domloadListener, {
        capture: true
      });
    } else {
      // Thunderbird 78
  ...
  registerListeners() {
    if (!this._loadListener) {
      ...
      if (this._isTB91) {
        ...
        window.browsingContext.embedderElement.addEventListener("DOMContentLoaded", this._domloadListener, {
          capture: true
        });
      } else {
        // Thunderbird 78.
  ...
  _onLoad(event) {
    if (event.target.documentURI == "about:blank" || event.target.browsingContext?.currentURI?.spec == "about:blank") {
      return;
    } // TODO: Handle BIDI
  ...
  onClickIframe(event) {
    ...
    if (this._isTB91 && event.target.ownerDocument.URL != this.iframe.contentDocument.URL) {
      return;
    }
...
MessageIFrame.propTypes = {
  ...
  id: (prop_types_default()).number.isRequired,
  isInTab: (prop_types_default()).bool.isRequired,
  isStandalone: (prop_types_default()).bool.isRequired,
...
class Message extends react.PureComponent {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.lastScrolledMsgUri != this.props.message.msgUri || prevProps.iframesLoading && !this.props.iframesLoading) {
      this.lastScrolledMsgUri = this.props.message.msgUri; // The additional setTimeout gives slightly longer for the event queue
      // to flush, and the message body to finish displaying - this helps the
      // conversation-in-tab view to scroll properly.

      setTimeout(() => {
        window.requestAnimationFrame(() => {
          window.scrollTo(0, // The header is 44px high (yes, this is hardcoded and ugly).
...
/* harmony export */   "c": () => (/* binding */ conversationUtils)
/* harmony export */ });
...
let conversationUtils = new class {
  async forward(tabId, msgs) {
    ...
    let displayedMsgs = await _es_modules_thunderbird_compat_js__WEBPACK_IMPORTED_MODULE_0__/* .browser.messageDisplay.getDisplayedMessages */ .Xh.messageDisplay.getDisplayedMessages(tabId);
    let identityId = undefined;

    if (displayedMsgs.length) {
      let accountId = displayedMsgs[0].folder.accountId;
      let account = await _es_modules_thunderbird_compat_js__WEBPACK_IMPORTED_MODULE_0__/* .browser.accounts.get */ .Xh.accounts.get(accountId);
      identityId = account.identities[0]?.id;
    }

    await _es_modules_thunderbird_compat_js__WEBPACK_IMPORTED_MODULE_0__/* .browser.compose.beginNew */ .Xh.compose.beginNew({
      identityId,
      isPlainText: false,
      body
    });
  ...
  async _exportConversationAsHtml(msgs) {
    ...
    let html = "<html><body>" + "<p>" + _es_modules_thunderbird_compat_js__WEBPACK_IMPORTED_MODULE_0__/* .browser.i18n.getMessage */ .Xh.i18n.getMessage("conversation.forwardFillInText") + "</p>" + hr;
    let promises = [];
  ...
  async _exportMsgAsHtml(msg) {
    ...
    let body = await _es_modules_thunderbird_compat_js__WEBPACK_IMPORTED_MODULE_0__/* .browser.conversations.quoteMsgHdr */ .Xh.conversations.quoteMsgHdr(msg.id); // UGLY HACK. I don't even wanna dig into the internals of the composition
...
/* harmony import */ var _es_modules_thunderbird_compat_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(2707);
/* harmony import */ var _reducer_compose_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(6000);
/* This Source Code Form is subject to the terms of the Mozilla Public
 * License, v. 2.0. If a copy of the MPL was not distributed with this
 * file, You can obtain one at https://mozilla.org/MPL/2.0/. */
