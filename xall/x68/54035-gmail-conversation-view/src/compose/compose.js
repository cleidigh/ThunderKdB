  ...
  sendMessage() {
    return async function (dispatch, getState) {
      ...
      await dispatch(composeSlice.actions.setSendStatus({
        sending: false,
        modified: false,
        sendingMsg: success ? "" : i18n.getMessage("compose.couldntSendTheMessage")
      }));
...
const Compose = ReactRedux.connect(state => ({
  details: state
}), {
...
export function Main() {
  const [fieldsInfo, setFieldsInfo] = React.useState(localize(INPUT_FIELDS, i18n));
  const [localizedSendButton, setLocalizedSendButton] = React.useState(localize("compose.send", i18n)); // When the i18n library is loaded, we want to translate all
