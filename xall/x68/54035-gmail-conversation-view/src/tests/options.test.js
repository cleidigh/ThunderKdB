...
const {
  ...
  browser
} = require("./utils"); // Import the components we want to test
...
describe("Option components have correct return values", () => {
  test("NumericOption always returns a numeric type", () => {
    ...
    const option = enzyme.shallow(React.createElement(NumericOption, {
      onChange: callback,
      name: "option_name",
      value: 7
    })); // Put in a number and expect it back

    option.find("input").simulate("change", {
      target: {
        value: "45"
      }
    });
    ...
    expect(typeof callback.mock.calls[0][1]).toBe("number"); // Put in a non-number and expect it to still return a number
  ...
  test("BinaryOption always returns a boolean type", () => {
    const callback = jest.fn();
    const option = enzyme.shallow(React.createElement(BinaryOption, {
      onChange: callback,
      name: "option_name",
      value: true
    }));
  ...
  test("TextOption always returns a string type", () => {
    const callback = jest.fn();
    const option = enzyme.shallow(React.createElement(TextOption, {
      onChange: callback,
      name: "option_name",
      value: "first text"
    }));
  ...
  test("ChoiceOption always returns the value supplied", () => {
    const callback = jest.fn();
    const option = enzyme.shallow(React.createElement(ChoiceOption, {
      onChange: callback,
      name: "option_name",
      choices: [{
        desc: "item1",
...
describe("Option Reducer and Actions tests", () => {
  ...
  test("initPrefs() retrieves preferences from `browser.storage.local`", async () => {
    await store.dispatch(actions.initPrefs()); // When we initialize preferences, there should be one call to "get"

    expect(mockedGet).toHaveBeenCalled(); // That call should have requested the "preferences" object

    expect(mockedGet.mock.calls[mockedGet.mock.calls.length - 1][0]).toBe("preferences");
  });
  test("savePref() sets a pref in `browser.storage.local`", async () => {
    await store.dispatch(actions.savePref("_custom_pref", 100)); // That call should have set a property on the "preferences" object

    expect(mockedSet.mock.calls[mockedSet.mock.calls.length - 1][0]).toMatchObject({
      preferences: {
        _custom_pref: 100
      }
    });
...
describe("Option full page tests", () => {
  ...
  test("Toggling an option changes the setting in browser.storage.local", async () => {
    ...
    const name = option.props().name; // We are going to click on the option and we expect that it's new value
    ...
    expect(afterChange[0]).toMatchObject({
      preferences: {
        [name]: true
      }
    });
