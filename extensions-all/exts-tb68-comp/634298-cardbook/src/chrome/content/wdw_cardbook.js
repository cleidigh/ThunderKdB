if ("undefined" == typeof(wdw_cardbook)) {
	...
	var myDirPrefIdDateFormat = cardbookRepository.getDateFormat(myDirPrefId, myDirPrefIdVCardVersion);
		...
		copyEntryFromTree: function () {
			...
			var tmpArray = textbox.id.split('_');
			var label = cardbookRepository.strBundle.GetStringFromName(tmpArray[0] + "Label");
			wdw_cardbook.copyFieldValue(tmpArray[0], label, tmpArray[1], "");
		},

		copyEntryFromEvent: function () {
			var textbox = document.popupNode.parentNode.parentNode;
			var fieldName = textbox.getAttribute('fieldName');
			var fieldValue = textbox.getAttribute('fieldValue');
			var label = cardbookRepository.strBundle.GetStringFromName(fieldName + "Label");
			wdw_cardbook.copyFieldValue(fieldName, label, "", fieldValue);
		},
