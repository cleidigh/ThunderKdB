if ("undefined" == typeof(wdw_mergeCards)) {
	...
	var wdw_mergeCards = {
		...
		createLabel: function (aRow, aName, aValue) {
			...
			aLabel.setAttribute('value', cardbookRepository.strBundle.GetStringFromName(aValue));
		},
				...
				for (var j = 0; j < cardbookRepository.customFields[i].length; j++) {
					if (wdw_mergeCards.addRowCustomFromArray(listOfCards, cardbookRepository.customFields[i][j][0])) {
						...
						for (var k = 0; k < listOfCards.length; k++) {
							var customValue = cardbookUtils.getCardValueByField(listOfCards[k], cardbookRepository.customFields[i][j][0], false);
							if (customValue != "") {
						...
						for (var k = 0; k < listOfCards.length; k++) {
							if (listOfCards[k][listOfFields[i]][j]) {
								...
								if (aPgType.length != 0 && aPgName != "") {
									...
									if (!found) {
										myDisplayedTypes.push(cardbookTypes.whichLabelTypeShouldBeChecked(listOfFields[i], listOfCards[k].dirPrefId, myInputTypes));
									}
								} else {
									myDisplayedTypes.push(cardbookTypes.whichLabelTypeShouldBeChecked(listOfFields[i], listOfCards[k].dirPrefId, myInputTypes));
								}
								if (listOfFields[i] == "impp") {
									...
									if (serviceCode != "") {
										...
										if (serviceLine[0]) {
											...
											wdw_mergeCards.createTextBox(aHbox, listOfFields[i] + '_' + j + '_' + k + '_Textbox0', cardbookUtils.formatTypesForDisplay(myDisplayedTypes), selected, true);
									...
									if (listOfFields[i] == "adr") {
										...
										wdw_mergeCards.createTextBox(aHbox, listOfFields[i] + '_' + j + '_' + k + '_Textbox1', cardbookUtils.cleanArray(aCardValue).join(" "), selected, true, listOfCards[k][listOfFields[i]][j]);
									}
								}
							} else {
								wdw_mergeCards.createHbox(aRow);
