	...
	var wdw_findDuplicates = {
		...
		generateCardArray: function (aCard) {
			try {
				...
				cardbookLog.updateStatusProgressInformation("wdw_findDuplicates.generateCardArray error : " + e, "Error");
			}
		},
		...
		compareCardArrayTry: function (aArray1, aArray2) {
			try {
				...
				cardbookLog.updateStatusProgressInformation("wdw_findDuplicates.compareCardArrayTry error : " + e, "Error");
			}
		},
		...
		compareCardArraySure: function (aArray1, aArray2) {
			try {
				...
				cardbookLog.updateStatusProgressInformation("wdw_findDuplicates.compareCardArraySure error : " + e, "Error");
			}
		},
			...
			try {
				...
				for (var i = 0; i < myCardArray.length-1; i++) {
					...
					if (myTmpResult.length > 1) {
						...
						cardbookUtils.sortCardsTreeArrayByString(myTmpResult, "uid", 1);
				...
				cardbookLog.updateStatusProgressInformation("wdw_findDuplicates.compareCards error : " + e, "Error");
			}
		},
		...
		displayResults: function () {
			...
			var buttonMergeLabel = cardbookRepository.strBundle.GetStringFromName("mergeCardsLabel");
			var buttonForgetLabel = cardbookRepository.strBundle.GetStringFromName("forgetCardsLabel");
			var buttonDeleteLabel = cardbookRepository.strBundle.GetStringFromName("deleteCardsLabel");
		...
		showLabels: function (aCount) {
			if (aCount == 0) {
				document.getElementById('noContactsFoundDesc').value = cardbookRepository.strBundle.GetStringFromName("noContactsDuplicated");
				...
				document.getElementById('numberContactsFoundDesc').value = cardbookRepository.strBundle.formatStringFromName("numberLines", [aCount], 1);
				document.getElementById('numberContactsFoundDesc').hidden = false;
			}
			if (wdw_findDuplicates.gHideForgotten) {
				document.getElementById('hideOrShowForgottenLabel').setAttribute('label', cardbookRepository.strBundle.GetStringFromName("showForgottenLabel"));
				document.getElementById('hideOrShowForgottenLabel').setAttribute('accesskey', cardbookRepository.strBundle.GetStringFromName("showForgottenAccesskey"));
			} else {
				document.getElementById('hideOrShowForgottenLabel').setAttribute('label', cardbookRepository.strBundle.GetStringFromName("hideForgottenLabel"));
				document.getElementById('hideOrShowForgottenLabel').setAttribute('accesskey', cardbookRepository.strBundle.GetStringFromName("hideForgottenAccesskey"));
			}				
		},
