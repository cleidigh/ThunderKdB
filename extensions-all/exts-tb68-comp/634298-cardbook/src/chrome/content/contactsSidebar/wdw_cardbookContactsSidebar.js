	...
	var wdw_cardbookContactsSidebar = {
		...
		search: function () {
			if (document.getElementById('cardbookpeopleSearchInput').value == "") {
				document.getElementById('cardbookpeopleSearchInput').placeholder = cardbookRepository.strBundle.GetStringFromName("cardbookSearchInputDefault");
			}
					...
					if (account[6] != "SEARCH") {
						if (cardbookRepository.verifyABRestrictions(myDirPrefId, searchAB, wdw_cardbookContactsSidebar.ABExclRestrictions, wdw_cardbookContactsSidebar.ABInclRestrictions)) {
									...
									for (var j in cardbookRepository.cardbookCardLongSearch[myDirPrefId]) {
										if (j.includes(searchInput) || searchInput == "") {
											for (let card of cardbookRepository.cardbookCardLongSearch[myDirPrefId][j]) {
												...
												if (card.isAList) {
													wdw_cardbookContactsSidebar.searchResults.push([cardbookUtils.getName(card), myDirPrefName,"", true, "LISTCARDBOOK", card, MailServices.headerParser.makeMimeAddress(card.fn, card.fn), '']);
												} else {
													if (card.emails != "") {
														var myFormattedEmails = [];
														myFormattedEmails = cardbookUtils.getMimeEmailsFromCards([card], useOnlyEmail);
														wdw_cardbookContactsSidebar.searchResults.push([cardbookUtils.getName(card), myDirPrefName, card.emails.join(', '), false, "CARDCARDBOOK", card, myFormattedEmails.join('@@@@@'), '']);
													}
												}
											}
										}
										...
										if (cardbookRepository.verifyCatRestrictions(myDirPrefId, category, searchInput, wdw_cardbookContactsSidebar.ABExclRestrictions,
											...
											for (let card of cardbookRepository.cardbookDisplayCards[myDirPrefId+"::categories::"+category].cards) {
												if (card.isAList) {
													...
													myFormattedEmails = myFormattedEmails.concat(cardbookUtils.getMimeEmailsFromCards([card], useOnlyEmail));
												}
											}
											if (myEmails != "") {
							...
							// One category
								...
								var myCategory = cardbookUtils.getNodeName(searchCategory);
						...
						if (cardbookRepository.verifyABRestrictions(myDirPrefId, searchAB, wdw_cardbookContactsSidebar.ABExclRestrictions, wdw_cardbookContactsSidebar.ABInclRestrictions)) {
							...
							if ((searchCategory === "allCategories") || (searchCategory === "noCategory") || (searchCategory === "onlyCategories")) {
								for (let card of cardbookRepository.cardbookDisplayCards[myDirPrefId].cards) {
									// All No categories
									if (searchCategory !== "onlyCategories") {
										var myDirPrefName = cardbookUtils.getPrefNameFromPrefId(card.dirPrefId);
		...
		startDrag: function (aEvent) {
			try {
				...
				cardbookLog.updateStatusProgressInformation("wdw_cardbookContactsSidebar.startDrag error : " + e, "Error");
			}
		},
		...
		deleteCard: function () {
			...
			for (var i = 0; i < listOfUid.length; i++) {
				if (listOfUid[i][0] === "CARDCARDBOOK" || listOfUid[i][0] === "LISTCARDBOOK") {
					...
					var myDirPrefId = cardbookUtils.getAccountId(listOfUid[i][1]);
					var myCategory = cardbookUtils.getNodeName(listOfUid[i][1]);
			...
			if (listOfUid.length != 0) {
				if (listOfUid.length != 1) {
					...
					if (listOfUid[0][0] == "CATCARDBOOK") {
						var myDirPrefId = cardbookUtils.getAccountId(listOfUid[0][1]);
		...
		onABChange: function (aParams) {
			...
			if (aParams) {
				...
				var myDirPrefId = cardbookUtils.getAccountId(aParams);
		...
		onSearchEntered: function () {
			...
			if (myABValue != "allAddressBooks") {
				if (document.getElementById('CardBookABMenulist').selectedItem.getAttribute('ABtype') == "standard-abook") {
					...
					if (!cardbookPreferences.getDBCached(myABValue) && cardbookUtils.isMyAccountRemote(cardbookPreferences.getType(myABValue))) {
						var mySearchValue = document.getElementById('cardbookpeopleSearchInput').value;
						if (cardbookUtils.isMyAccountSyncing(myABValue) || mySearchValue == "") {
							return;
						}
						cardbookPreferences.setLastSearch(myABValue, mySearchValue);
						cardbookSynchronization.searchRemote(myABValue, mySearchValue);
					} else {
						wdw_cardbookContactsSidebar.search();
					}
				}
